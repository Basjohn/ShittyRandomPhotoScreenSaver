    ...
    def render_blockspin_shader(self, target: QRect, state: Any) -> None:
        ...
        try:
            ...
            if state.direction in (SlideDirection.UP, SlideDirection.DOWN):
                axis_mode = 1
            elif state.direction in (SlideDirection.DIAG_TL_BR, SlideDirection.DIAG_TR_BL):
                axis_mode = 2  # Diagonal axis mode
            if pipeline.u_axis_mode_loc != -1:
                gl.glUniform1i(pipeline.u_axis_mode_loc, int(axis_mode))

            spin_dir = 1.0
            if state.direction in (SlideDirection.RIGHT, SlideDirection.DOWN, SlideDirection.DIAG_TR_BL):
                spin_dir = -1.0
    ...
    def render_peel_fallback(self, painter: QPainter, target: QRect, state: Any) -> None:
        ...
        for i in range(strips):
            if state.direction in (SlideDirection.LEFT, SlideDirection.RIGHT):
                ...
                # Diagonal: use horizontal strips with diagonal motion
            ...
            if local > 0.0:
                if state.direction in (SlideDirection.LEFT, SlideDirection.RIGHT):
                    ...
                    # Diagonal motion: move both x and y
                    sign_x = 1 if state.direction == SlideDirection.DIAG_TR_BL else -1
                    sign_y = 1  # Always move down for diagonal
                    max_offset_x = w + base_rect.width()
                    max_offset_y = h + base_rect.height()
                    dx = int(sign_x * max_offset_x * local)
                    dy = int(sign_y * max_offset_y * local)
                else:
