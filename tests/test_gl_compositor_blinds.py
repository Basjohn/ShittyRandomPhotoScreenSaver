"""Tests for GLCompositorWidget blinds transitions.

These tests validate that the compositor-based blinds path does not produce
...
def test_gl_compositor_blinds_no_underlay_and_no_black(qapp):
    """Compositor-driven blinds should not leak underlay or show black frames.

    Mirrors the compositor crossfade/slide/wipe tests but drives a blinds
    reveal between solid-colour images via GLCompositorBlindsTransition.
    """
    ...
    # GLCompositorBlindsTransition expects on its widget.
    ...
    trans = GLCompositorBlindsTransition(
        duration_ms=400,
        slat_rows=5,
        slat_cols=7,
    )
    ...
    try:
        ...
        started = False
    if not started:
        pytest.skip("GL compositor blinds could not be started in this environment")

    try:
        comp.makeCurrent()
    except Exception:
        pytest.skip("GL context not available for GLCompositorWidget (blinds)")
    ...
    for img in frames:
        ...
        ), f"GL compositor blinds underlay leak detected ({underlay_fraction:.2%})"

        # No mostly-black frames while transitioning between non-black colours.
        assert (
            dark_fraction < 0.7
        ), f"GL compositor blinds produced blank/very dark frame ({dark_fraction:.2%})"
