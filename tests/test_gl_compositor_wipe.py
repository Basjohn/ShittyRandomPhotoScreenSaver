"""Tests for GLCompositorWidget wipe transitions.

These tests validate that the compositor-based wipe path does not produce
...
def test_gl_compositor_wipe_no_underlay_and_no_black(qapp, direction):
    """Compositor-driven wipe should not leak underlay or show black frames.

    Mirrors the compositor crossfade/slide tests but drives a wipe between
    solid-colour images via GLCompositorWipeTransition.
    """
    ...
    # GLCompositorWipeTransition expects on its widget.
    ...
    trans = GLCompositorWipeTransition(
        duration_ms=300,
        direction=direction,
        easing="Auto",
    )
    ...
    try:
        ...
        started = False
    if not started:
        pytest.skip("GL compositor wipe could not be started in this environment")

    try:
        comp.makeCurrent()
    except Exception:
        pytest.skip("GL context not available for GLCompositorWidget (wipe)")

    # Capture a series of frames from the parent while the compositor wipes.
    frames: list[QImage] = []
    for _ in range(24):
        qapp.processEvents()
    ...
    for img in frames:
        ...
        ), f"GL compositor wipe underlay leak detected ({underlay_fraction:.2%}) for {direction.name}"

        # No mostly-black frames while transitioning between non-black colours.
        assert (
            dark_fraction < 0.7
        ), f"GL compositor wipe produced blank/very dark frame ({dark_fraction:.2%}) for {direction.name}"
