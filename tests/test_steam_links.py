...
def test_friend_profile_target_is_validated_and_browser_safe():
    target = friend_profile_target("76561198000000001")
    assert target is not None
    assert target.kind == "friend_profile"
    assert target.steam_url == target.browser_url
    assert target.browser_url == (
        "https://steamcommunity.com/profiles/76561198000000001"
    )
    assert friend_profile_target("7656/not-safe") is None
