...
def test_production_widget_activation_queues_backend_and_cache_prepare(
    qt_app,
    monkeypatch,
) -> None:
    from widgets.gmail_widget import GmailWidget

    previous_backend = GmailBackend._instance
    previous_oauth = GmailOAuthManager._instance
    GmailBackend._instance = None
    GmailOAuthManager._instance = None
    manager = _QueuedIoManager()
    monkeypatch.setattr(
        "widgets.gmail_widget.automatic_service_updates_enabled",
        lambda: False,
    )
    ...
    try:
        widget = GmailWidget()
        assert widget._backend.is_initialized is False
        widget.set_thread_manager(manager)  # type: ignore[arg-type]
        widget._activate_impl()

        assert [task.category for task in manager.tasks] == [
            "gmail_backend_bootstrap",
            "gmail_startup_cache",
        ]
    finally:
        if widget is not None:
            widget.cleanup()
        GmailBackend._instance = previous_backend
        GmailOAuthManager._instance = previous_oauth


def test_widget_waits_for_backend_then_starts_pending_fetch(
    qt_app,
) -> None:
    from widgets.gmail_widget import GmailWidget
    ...
    client = object()
    widget = GmailWidget()
    widget._backend = backend  # type: ignore[assignment]
    widget._backend_ready = False
    widget.set_thread_manager(manager)  # type: ignore[arg-type]

    try:
        assert widget._fetch_emails(defer_for_transition=False) is True
        assert widget._pending_fetch_after_backend_ready is True
        ...
        assert widget._backend_ready is True
        assert widget._gmail_client is client
        assert len(manager.tasks) == 1
        assert manager.tasks[0].category == "gmail_fetch"
    finally:
        widget.cleanup()


def test_late_backend_ready_callback_is_rejected_after_widget_deactivation(
    qt_app,
) -> None:
    from widgets.gmail_widget import GmailWidget
    ...
    widget._backend = backend  # type: ignore[assignment]
    widget._backend_ready = False
    widget.set_thread_manager(_QueuedIoManager())  # type: ignore[arg-type]

    try:
        widget._begin_backend_initialization()
        ...
        assert widget._backend_ready is False
        assert widget._gmail_client is None
    finally:
        widget.cleanup()
