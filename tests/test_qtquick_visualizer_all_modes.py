...
def test_capture_racing_mode_replacement_is_discarded_without_publication(
    monkeypatch,
    race_point: str,
) -> None:
    from widgets.spotify_visualizer import legacy_render_snapshot_adapter
    from widgets.spotify_visualizer.tick_pipeline import _publish_logical_state

    race_reached = threading.Event()
    ...
    widget = SimpleNamespace(
        ...
        _last_engine_generation_seen=5,
        _last_engine_activation_seen=7,
        _spotify_playing=True,
        _has_pushed_first_frame=True,
        _display_bars=(
            _BlockingBars()
            if race_point == "after_resolve"
            else (0.2, 0.6)
        ),
        _bar_count=2,
        _display_bars_source_generation=5,
        _display_bars_source_activation=7,
        _logical_mailbox=mailbox,
        _logical_runtime=None,
    )
    monkeypatch.setattr(
        legacy_render_snapshot_adapter,
        "_base_extras",
        lambda _widget, _mode_id, _engine: {},
    )
    if race_point == "before_resolve":
        def _blocking_source_identity(_widget, _engine, _mode_id):
            race_reached.set()
            if not release_capture.wait(timeout=1.0):
                raise TimeoutError("test capture was not released")
            return 5, 7, 1.99
    ...
    thread.start()
    assert race_reached.wait(timeout=1.0)
