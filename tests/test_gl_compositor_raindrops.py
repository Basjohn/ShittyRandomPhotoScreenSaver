"""Tests for GLCompositorWidget Rain Drops transition.

These tests validate that the compositor-based Rain Drops path does not
produce mostly-black frames or expose a coloured underlay when driven in a
simple QWidget hierarchy. This exercises the GLSL-backed path when PyOpenGL
is available, and falls back to the diffuse/QPainter implementation otherwise.
"""
...
def test_gl_compositor_raindrops_no_underlay_and_no_black(qapp):
    """Compositor-driven Rain Drops should not leak underlay or show black frames.

    Mirrors the compositor crossfade/diffuse/block spins tests but drives a
    Rain Drops transition between solid-colour images via
    ...
    # GLCompositorRainDropsTransition expects on its widget.
    ...
    trans = GLCompositorRainDropsTransition(
        duration_ms=700,
        easing="Auto",
    )
    ...
    try:
        ...
        started = False
    if not started:
        pytest.skip("GL compositor Rain Drops could not be started in this environment")

    try:
        comp.makeCurrent()
    except Exception:
        pytest.skip("GL context not available for GLCompositorWidget (Rain Drops)")
    ...
    for img in frames:
        ...
        ), f"GL compositor Rain Drops underlay leak detected ({underlay_fraction:.2%})"

        # No mostly-black frames while transitioning between non-black colours.
        assert (
            dark_fraction < 0.7
        ), f"GL compositor Rain Drops produced blank/very dark frame ({dark_fraction:.2%})"
