...
class TestSpectrumShapeConfig:
    ...
    def test_quick_bar_layout_reclaims_width_without_centering_waste(self):
        from rendering.quick.visualizer.implementations.spectrum import (
            compute_quick_spectrum_layout,
        )

        # The retained Quick owner receives the visible content width directly;
        # 889px is the old 900px card extent after its retired 11px shrink.
        layout = compute_quick_spectrum_layout(
            local_content_rect=(0.0, 0.0, 889.0, 180.0),
            viewport_extent=(889.0, 180.0),
            visual_scale=1.0,
            bar_count=33,
        )
        assert layout.bars_left == pytest.approx(9.0)
        assert 889.0 - (layout.bars_left + layout.bar_span) == pytest.approx(11.0)
        assert layout.bar_span == pytest.approx(869.0, abs=1e-6)
