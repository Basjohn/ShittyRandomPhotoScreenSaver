"""Tests for GLCompositorWidget and GLCompositorCrossfadeTransition.

These tests validate that the compositor-based crossfade path does not
produce mostly-black frames or expose a coloured underlay when driven
in a simple QWidget hierarchy.
"""
...
def test_gl_compositor_crossfade_no_underlay_and_no_black(qapp):
    """Compositor-driven crossfade should not leak underlay or show black frames.

    This is similar in spirit to the overlay tests, but targets the new
    GL compositor path directly: a parent with a loud magenta background,
    a compositor child covering the parent, and a controller transition
    driving a crossfade between solid red and blue images.
    """
    ...
    # GLCompositorCrossfadeTransition expects on its widget.
    ...
    trans = GLCompositorCrossfadeTransition(duration_ms=300, easing="Auto")
    ...
    try:
        ...
        started = False
    if not started:
        pytest.skip("GL compositor crossfade could not be started in this environment")

    try:
        comp.makeCurrent()
    except Exception:
        pytest.skip("GL context not available for GLCompositorWidget")

    # Capture a series of frames from the parent while the compositor crossfades.
    frames: list[QImage] = []
    for _ in range(20):
        qapp.processEvents()
    ...
    for img in frames:
        ...
        ), f"GL compositor underlay leak detected ({underlay_fraction:.2%})"

        # No mostly-black frames while transitioning between non-black colours.
        assert (
            dark_fraction < 0.7
        ), f"GL compositor produced blank/very dark frame ({dark_fraction:.2%})"
