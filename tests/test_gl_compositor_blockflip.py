"""Tests for GLCompositorWidget block puzzle flip transitions.

These tests validate that the compositor-based block flip path does not
produce mostly-black frames or expose a coloured underlay when driven in a
simple QWidget hierarchy.
"""
...
def test_gl_compositor_blockflip_no_underlay_and_no_black(qapp):
    """Compositor-driven block flip should not leak underlay or show black frames.

    Mirrors the compositor crossfade/slide/wipe tests but drives a block
    puzzle flip between solid-colour images via GLCompositorBlockFlipTransition.
    """
    ...
    # GLCompositorBlockFlipTransition expects on its widget.
    ...
    trans = GLCompositorBlockFlipTransition(
        duration_ms=400,
        grid_rows=4,
        grid_cols=6,
    ...
    try:
        ...
        started = False
    if not started:
        pytest.skip("GL compositor block flip could not be started in this environment")

    try:
        comp.makeCurrent()
    except Exception:
        pytest.skip("GL context not available for GLCompositorWidget (block flip)")

    # Capture a series of frames from the parent while the compositor flips.
    frames: list[QImage] = []
    for _ in range(28):
        qapp.processEvents()
    ...
    for img in frames:
        ...
        ), f"GL compositor block flip underlay leak detected ({underlay_fraction:.2%})"

        # No mostly-black frames while transitioning between non-black colours.
        assert (
            dark_fraction < 0.7
        ), f"GL compositor block flip produced blank/very dark frame ({dark_fraction:.2%})"
