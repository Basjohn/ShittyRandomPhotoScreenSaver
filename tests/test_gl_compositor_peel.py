"""Tests for GLCompositorWidget peel transitions.

These tests validate that the compositor-based + GLSL peel path does not
...
def test_gl_compositor_peel_no_underlay_and_no_black(qapp):
    """Compositor-driven Peel should not leak underlay or show black frames.

    Mirrors the compositor crossfade/slide/wipe and blockflip tests but drives
    a Peel transition between solid-colour images via GLCompositorPeelTransition.
    """
    ...
    # GLCompositorPeelTransition expects on its widget.
    ...
    trans = GLCompositorPeelTransition(
        duration_ms=700,
        strips=10,
    )
    ...
    try:
        ...
        started = False
    if not started:
        pytest.skip("GL compositor peel could not be started in this environment")

    try:
        comp.makeCurrent()
    except Exception:
        pytest.skip("GL context not available for GLCompositorWidget (peel)")

    # Capture a series of frames from the parent while the compositor peels.
    frames: list[QImage] = []
    for _ in range(32):
        qapp.processEvents()
    ...
    for img in frames:
        ...
        ), f"GL compositor peel underlay leak detected ({underlay_fraction:.2%})"

        # No mostly-black frames while transitioning between non-black colours.
        assert (
            dark_fraction < 0.7
        ), f"GL compositor peel produced blank/very dark frame ({dark_fraction:.2%})"
