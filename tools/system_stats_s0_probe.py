...
samples directly for a bounded experiment so admission can be decided before a
widget, Settings key, or QML component exists.
"""
...
from core.system_stats.source import PersistentWindowsGpuAdapterSource, WholeSystemCpuRamSource
...
def _summary(values_ms: list[float]) -> dict[str, float | int]:
    ...
    return {"samples": len(ordered), "p50_ms": percentile(0.50), "p95_ms": percentile(0.95), "max_ms": max(ordered, default=0.0)}
...
def _measure(condition: str, samples: int, interval_seconds: float) -> dict[str, Any]:
    ...
    if gpu is not None:
        gpu.close()
    return {"condition": condition, "timing": _summary(durations), "observations": statuses}


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Bounded System Stats S0 source probe")
    parser.add_argument("--condition", choices=("baseline", "cpu-ram", "gpu", "all"), default="all")
    ...
    conditions = ("baseline", "cpu-ram", "gpu") if args.condition == "all" else (args.condition,)
    total_seconds = len(conditions) * max(0, samples - 1) * interval + 5.0
    workers = max(0, int(args.contention_workers))
    result: dict[str, Any] = {"samples": samples, "interval_seconds": interval, "contention_workers": workers, "conditions": []}
    with ProcessPoolExecutor(max_workers=workers) if workers else _NullExecutor() as executor:
