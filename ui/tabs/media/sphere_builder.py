...
def build_sphere_ui(tab, parent_layout) -> None:
    ...
    _, surface = build_collapsible_bucket(tab, scaffold.normal_layout, mode_key="sphere",
        bucket_key="surface", title="Surface", helper_text="Choose the material, lighting, and surface finish for the sphere.")
    _, motion = build_collapsible_bucket(tab, scaffold.normal_layout, mode_key="sphere",
        bucket_key="motion", title="Motion", helper_text="Tune rotation, deformation, and the gentle motion shown while music is quiet.")
    ...
    tab.sphere_material.setCurrentText(tab._default_str("spotify_visualizer", "sphere_material"))
    ...
    tab.sphere_light_direction.setCurrentText(tab._default_str("spotify_visualizer", "sphere_light_direction").upper())
    ...
    slider(surface, "sphere_surface_detail", "sphere_surface_detail", "Bump Strength:", 200, "", 100.0)
    ...
    tab.sphere_vocal_response.setToolTip("Shapes the vocal-frequency range (a mid/high blend); it does not isolate voices. The upper range is intentionally strong for expressive deformation.")
    slider(motion, "sphere_bump_reactivity", "sphere_bump_reactivity", "Bump Reactivity:", 200, "", 100.0)
    ...
    slider(surface, "sphere_material_fx", "sphere_material_fx", "Material Effects:", 200, "", 100.0)
    _widget, content = row(surface, "Anti-Aliasing:")
    tab.sphere_antialiasing = QCheckBox("Enabled")
    tab.sphere_antialiasing.setChecked(tab._default_bool("spotify_visualizer", "sphere_antialiasing"))
    ...
    tab.sphere_shadow_enabled.setChecked(tab._default_bool("spotify_visualizer", "sphere_shadow_enabled"))
    ...
    slider(surface, "sphere_shadow_strength", "sphere_shadow_strength", "Shadow Darkness:", 100, "", 100.0)
    tab.sphere_shadow_strength.setToolTip("Controls the opacity of the lighting-direction cast shadow.")
    slider(motion, "sphere_rotation_speed", "sphere_rotation_speed", "Rotation:", 200, "x", 100.0)
    slider(motion, "sphere_idle_motion", "sphere_idle_motion", "Idle Motion:", 100, "", 100.0)
