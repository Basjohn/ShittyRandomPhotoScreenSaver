...
| tools/visualizer_preset_repair.py | DPI-aware GUI + CLI batch runner. Repairs curated preset JSON/SST files (prunes junk keys, rewrites lean payloads with a single `snapshot.widgets.spotify_visualizer` block, preserves/infers `preset_index`, backfills mandatory per-mode technical + critical visual keys, and strips retired authored keys like `energy_boost`/`use_raw_energy` from clean payloads), writes `.bak*` backups, supports undo, and offers `--repair-all`, `--audit-curated`, and `--reindex-curated` for repo-wide preset hygiene. Tests treat curated packs as fluid authored content except for structural guards such as slot uniqueness, schema hygiene, and the intentional `Preset 1` baseline fence. |
...
| Vis Signal Policy | widgets/spotify_visualizer/config_applier.py | _resolve_continuous_energy_bands(), build_gpu_push_extra_kwargs() | Shared visualizer signal handoff. All modes use the canonical smoothed continuous-energy path by default; pre-AGC selection is now a runtime-only/debug seam rather than persisted settings schema, and Blob must not grow a sidecar signal path. |
...
| Vis Preset Manifest | core/visualizer_preset_manifest.py | load_curated_visualizer_preset_manifest(), resolve_curated_visualizer_manifest_entries(), scan_curated_visualizer_preset_tree(), sync_curated_preset_tree(), write_curated_visualizer_preset_manifest() | Shipped curated-preset cleanup + reconciliation helpers. Frozen stale-file cleanup still uses the manifest contract, while source-tree-aware flows (such as Replace Visualizers) reconcile against the actual curated tree, then rewrite the target manifest from that reconciled view so newly authored shipped presets are auto-accepted without leaving stale manifest metadata behind. |
...
| Blob | renderers/blob.py | Ghost alpha, blob colours, body response/release, glow, smoothed/peak energy, per-band peak (bass/mid/high/overall) for SDF ghost shape, idle/audio edge motion, single outward stretch control, and Blob Shaper profile/routing upload with editor-aligned top-origin angular sampling, reaction-canvas-authoritative energy routing, broadened cosine-smoothed shaper routing fields, blended stage/live shaper-drive bands for calmer deformation response, creator-path wiring for authored runtime controls, gain-boosted shaper drive for moderate live energy, and idle wobble suppression so authored shaper silhouettes remain authoritative at rest |
...
| Shared Styles | ui/tabs/shared_styles.py | NoWheelSlider, SPINBOX_STYLE, COMBOBOX_STYLE, SLIDER_STYLE, CIRCLE_CHECKBOX_STYLE, TOOLTIP_STYLE, SCROLL_AREA_STYLE, SUBSECTION_DIVIDER_STYLE, SUBSECTION_DIVIDER_TITLE_STYLE, style_group_box() | Centralised QSS constants, shared widgets (NoWheelSlider), and `style_group_box()` helper for seam-free QGroupBox borders across all tabs |
...
| tests/test_visualizer_preset_manifest.py | Shipped curated-preset manifest parity plus stale-file sync policy | 
