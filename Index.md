...
| CUSTOM layout session manager | `rendering/custom_layout_manager.py` | Shell-driven CUSTOM edit session lifecycle, live screen-binding sync, temporary shell orchestration, save/cancel flow, authored-layout reset flow, runtime-update deferral, numbered-monitor ownership transfer between displays, and canonical widget rebuild on save/reset so all widget families return through the same runtime setup path |
...
| Media widget | `widgets/media_widget.py` | Now Playing overlay with canonical local smart-poll timer reset and pending playback-state debounce reset ownership, plus retained-display/runtime-state handling and media control feedback |
