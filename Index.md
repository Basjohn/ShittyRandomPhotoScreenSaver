...
| Bubble simulation | `widgets/spotify_visualizer/bubble_simulation.py` | CPU-side Bubble particle simulation, two-lane Bubble sizing/gating (hero big lane plus small/medium lane), display-only big/hero visual smoothing at the rendered-radius seam via the authored `bubble_big_visual_smoothing` control, live size-envelope adaptation for existing bubbles, and per-frame big-lane diagnostics used by the authored Deep Sea regression bars |
