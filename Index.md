...
| tools/visualizer_preset_repair.py | DPI-aware GUI + CLI batch runner. Repairs curated preset JSON/SST files (prunes junk keys, rewrites lean payloads with a single `snapshot.widgets.spotify_visualizer` block, preserves/infers `preset_index`, backfills mandatory per-mode technical + critical visual keys, and strips retired authored keys like `energy_boost`/`use_raw_energy` from clean payloads), writes `.bak*` backups, supports undo, and offers `--repair-all`, `--audit-curated`, and `--reindex-curated` for repo-wide preset hygiene. Primary shipped modes now maintain a six-slot curated pack baseline guarded by tests. |
...
| Vis Preset Manifest | core/visualizer_preset_manifest.py | load_curated_visualizer_preset_manifest(), sync_curated_preset_tree() | Shipped curated-preset source-of-truth for stable onefile extraction trees. Removes stale shipped preset files in frozen builds without mutating repo/script trees by default. |
...
| Blob Shape Editor | ui/tabs/media/blob_shape_editor.py | BlobShapeEditor, _PolarEditorCanvas, _EnergyChip | Spatial energy routing editor: two side-by-side polar canvases (base shape + reaction limit) with draggable profile nodes and copy-on-drag energy source palette (bass/mid/vocals/treble/transient) |
...
| Reddit Helper Worker | helpers/reddit_helper_worker.py | main(), process_queue(), _run_watcher(), _acquire_watcher_singleton() | Interactive desktop worker: opens URLs, handles open_settings action + completion tokens, writes heartbeat, canonicalizes retry/backoff, expires stale queue entries safely, self-exits for session-scoped launches once the owner is gone and the queue is idle, and enforces a per-session watcher singleton so duplicate helpers do not stack. |
