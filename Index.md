...
| Technical Controls | ui/tabs/media/technical_controls.py | build_per_mode_technical_group, register_per_mode_technical_controls, load_per_mode_technical_controls, collect_per_mode_technical_controls, get_per_mode_controls, _TRANSIENT_MIX_META | Shared per-mode Technical settings group: sensitivity, adaptive sensitivity, dynamic range, dynamic floor, manual floor, audio block size (Auto/128/256/512/1024 with negotiated-size logging), bar count, **input_gain** (5%–200% pre-FFT virtual volume), **agc_strength** (0%–100%), **kick_lane_gain** (0%–200% kick express lane), **transient_pulse_gain** (0%–300% transient pulse mix), **transient_clamp** (0%–300% max transient bass), **per-mode transient mix sliders** (§2.3): spectrum_lane_transient_mix (kick lane feed), bubble_transient_mix_bass/vocal (pulse weights), blob_transient_mix_bass/vocal (deformation weights), sine_wave_transient_width_mix (width reaction), oscilloscope_transient_width_mix (sensitivity boost). ~~energy_boost~~ and ~~use_raw_energy~~ deprecated/hidden (preset compat retained). Load/save wired via per-mode cache. |
