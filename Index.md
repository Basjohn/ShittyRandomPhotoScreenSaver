...
| `Docs/Steam_Data_Feasibility.md` | Steam source confidence matrix and promotion/gate contract for Achievement Pulse and the unfinished Steam card prototypes |
| `Docs/SRPSS_Steam_Widget_Family_Implementation_Plan.md` | Detailed phased Steam-family implementation plan; Achievement Pulse is the engineering/customization chassis while future cards retain distinct visual identities |
...
| Steam asset cache | `core/steam/assets.py` | Injected-fetcher Steam artwork/avatar/achievement-icon HTTPS host and signature validation, atomic cache writes, and bounded local asset eviction |
...
| Steam card widget | `widgets/steam_card_widget.py` | Steam card overlays that consume resolved/mock view models through shared painter helpers; public Achievement Pulse preloads cache before first fade, schedules bounded schema/game-art/achievement-icon IO after fade using the shared freshness window, caches DPR-aware local image scaling, and accepts transition-safe manual refresh while constructors and paint remain provider-inert; unfinished cards remain dev-gated |
| Steam settings shell | `ui/tabs/widgets_tab_steam.py` | Lazy public Steam Settings section with a true family master, nested card Layout/Appearance/Content buckets, explicit OpenID/API-key connect controls, safe encrypted-storage status hydration, DPAPI-safe disconnect actions, family font controls, bracketed cache-only recent-game choice names, and Achievement Pulse game-art/latest-achievement-artwork/shape/bounded-square-size/one-to-five-unlock/field/all-field-double-capsule/capsule-font/RGBA preferences; unfinished card buckets remain hidden without `--devsteam`, opening general Settings remains inert, and opening Steam never decrypts credentials or contacts Steam |
...
| Widget factories | `rendering/widget_factories.py` | Factory registry and per-family factory classes; Achievement Pulse is registered normally, while the three unfinished Steam card factories remain behind the named `--devsteam` gate and provider/cache/credential inert until promotion |
