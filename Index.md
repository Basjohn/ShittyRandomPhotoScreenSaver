...
- rendering/widget_manager.py
  ...
  - **Jan 30, 2026 Fix**: Spotify secondary fade starters now defer until the anchor media card reports `isVisible()==True`, retrying with a capped backoff so the visualizer/volume never miss the initial fade wave on cold boot.
...
- widgets/spotify_volume_widget.py
  ...
  - **Coordinated Fade (Dec 2025)**: Now uses `request_overlay_fade_sync()` to participate in primary overlay fade wave, fading in simultaneously with other widgets instead of using delayed secondary fade. Jan 2026 added anchored deferral identical to the visualizer so the secondary fade starter waits for the media card to finish its primary fade before showing the control.
