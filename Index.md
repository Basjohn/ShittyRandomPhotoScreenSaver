...
| Defaults SST regenerator | `tools/regenerate_sst_defaults.py` | Canonical Normal/MC SST export through an explicit temporary SettingsManager storage root; never reads, migrates, resets, or writes installed profile JSON |
...
| Steam Abandonment resolver | `core/steam/abandonment_issues.py` | Pure verified-timestamp eligibility plus 15-minute floor, 26-week/sub-2-hour/cache-only-0-2-unlock preference tiers, all-unlocked demotion without completion claims, cached latest-unlock provenance, pinned/unavailable behavior, and preference-biased deterministic random selection with immediate-repeat exclusion and no provider/Qt dependency |
| Steam Abandonment cache bridge | `core/steam/abandonment_cache.py` | Cache-first owned/recent resolution, at most 12 exact-path achievement hints prioritized current/pinned-first (no scan/request), 24-hour library/shared recent freshness, profile-shared policy-fingerprinted exposure/rotation state plus persisted draw counter and remaining-due calculation from the shared Steam refresh interval, cache-only non-sequential interval/forced semantic advances with app/archive diagnostics, and cached-library Settings choices |
| Steam card visual components | `widgets/steam_components.py` | Provider-inert Steam view models, deterministic authored layout metrics, measured occupancy-aware field planning, paint fingerprints, and shared painter helpers that keep `Custom` as uniform scale instead of content authority; Achievement Pulse owns header-aligned Wide/Square/default Portrait game art, a widened centered-below-art Unlocked rail with local font fitting rather than ordinary-count elision, a one-to-five unprefixed latest-unlock hierarchy, an optional 40px primary icon positioned after measured unlock text, colon-free compact capsules, and default-on all-field double headings/fitted values with `PREVIOUSLY`, independent capsule-font growth, RGBA styling, and exterior shadows |
...
| Steam Abandonment visual components | `widgets/steam_abandonment_components.py` | Provider-inert authored archival layout and renderer: large cover shelf, warm archive tab, title, optional stable 60/40 rediscovery message, reliable `LAST VISIT` stamp, evidence-gated achievements/latest-unlock/exact-last-played/archive-class shelves, whole-row adaptive ledger growth, Custom scaling, and dynamic-content opacity isolation |
| Steam Abandonment widget | `widgets/abandonment_issues_widget.py` | Production cache-first Abandonment overlay with worker-owned cache/provider/art preparation, profile-shared cache-only semantic rotation driven solely by `widgets.steam.refresh_minutes` whose remaining interval survives rebuilds and setting changes, selected public-art cache-miss hydration plus one definitive-miss shape fallback on the existing IO job when updates are allowed, forced non-repeating manual draws, shared transition-deferral retry for collided expiries/manual refresh, DPR-local paint assets, and atomic fade commits of game/title/art |
