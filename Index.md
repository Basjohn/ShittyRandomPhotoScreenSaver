...
| Sine Wave | widgets/spotify_visualizer/shaders/sine_wave.frag | u_line_color, u_glow_*, u_reactive_glow, u_sensitivity, u_osc_speed, u_osc_sine_travel, u_sine_travel_line2, u_sine_travel_line3, u_card_adaptation, u_playing, u_line_count, u_line{2,3}_{color,glow_color}, u_bass/mid/high_energy, u_wave_effect, u_micro_wobble, u_osc_vertical_shift, u_heartbeat, u_heartbeat_intensity, u_rainbow_enabled, u_rainbow_hue_offset, u_width_reaction | Sine wave visualizer: audio-reactive amplitude, card adaptation, multi-line (up to 3) with tight phase offsets (0.315/0.90 radians), playback-gated oscillation, wave effect, micro wobble (smooth snake-like bass-driven), heartbeat (crest-locked spikes sampled via density-aware search with 300 ms CPU decay envelope), rainbow hue cycling with **line colours exempted** (only glow tones shift), vertical shift, width reaction, bass_pulse×1.1 sensitivity. Glow accumulation uses a per-line premultiplied glow budget (shared helper with oscilloscope) so intersections never exceed 1.0 alpha. |
