...
| Module | File | Role |
|---|---|---|
| Painted frame shadows | `widgets/base_overlay_widget.py` | Settings-controlled cached painter-drawn card, text, and header shadow path for framed runtime overlay widgets, plus the shared reassertion seam that keeps saved CUSTOM rects authoritative when live content sizing tries to resize a custom-positioned overlay |
