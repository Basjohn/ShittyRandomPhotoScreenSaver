...
| tools/visualizer_preset_repair.py | DPI-aware GUI + CLI batch runner. Repairs curated preset JSON/SST files (prunes junk keys, rewrites lean payloads with a single `snapshot.widgets.spotify_visualizer` block, preserves/infers `preset_index`, backfills mandatory per-mode technical + critical visual keys, promotes stale non-mirrored Spectrum `Low/Mid` lane families into the explicit `Low-Mid/Vocal` family without flattening boundary drift, derives new Spectrum lane-strength maps from legacy scalar controls, and strips retired authored keys like `energy_boost`/`use_raw_energy` from clean payloads), keeps at most two rotating backups per preset, persists undo state under `temp/visualizer_preset_backups/undo_state.json`, supports undo, and offers `--repair-all`, `--audit-curated`, and `--reindex-curated` for repo-wide preset hygiene. Tests treat curated packs as fluid authored content except for structural guards such as slot uniqueness, schema hygiene, and the intentional `Preset 1` baseline fence. |
