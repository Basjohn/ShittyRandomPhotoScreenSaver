...
| SRPSS_ENABLE_GENERAL_PRESETS | false | Enable the legacy global settings-dialog Presets tab/system (`core/settings/presets.py`). When unset, global presets are effectively forced to Custom. Visualizer presets are unaffected. |
...
| Presets | core/settings/presets.py | PresetDefinition, apply_preset(), are_general_presets_enabled() | General/global widget presets system (moved from core/presets.py). Disabled by default unless `SRPSS_ENABLE_GENERAL_PRESETS=1`; this does not affect visualizer presets. |
| Vis Presets | core/settings/visualizer_presets.py | VisualizerPreset, get_presets(), apply_preset_to_config(), get_packaged_visualizer_presets_dir() | Per-visualizer-mode preset registry. Script mode uses the repo `presets/visualizer_modes/<mode>` tree directly; frozen SCR and MC builds converge on the shared ProgramData curated tree, including one-dir extraction-path variants (`onefile*`) so runtime never treats extracted trees as active curated roots. Packaged/bundled preset assets remain replacement/bootstrap sources only. Curated slot identity uses filename slot/name precedence (`preset_N_suffix.json`) so slider labels and `Edit Preset` targets stay in sync; snapshot overrides can replace settings for a slot but do not rename curated slot labels. |
...
| Bubble Simulation | widgets/spotify_visualizer/bubble_simulation.py | BubbleSimulation, BubbleState | CPU-side particle simulation for bubble mode; tick()/snapshot() on COMPUTE thread pool, coalesced results posted to UI thread; snapshot() returns (pos_data, extra_data, trail_data); trail records 3 previous positions per bubble (TRAIL_STEPS=3); drift system includes **Swish (Horizontal/Vertical)** and **Swirl (Clockwise/Counter-Clockwise)**. Collision system now supports class-scoped bounce probability/speed, same-class-only collision mode, collision-pop policies (`off`/`one`/`all`), pulse-aware collision radii, and drift-aware post-bounce stabilization (cooldown + short glide damping). |
| Config Applier | widgets/spotify_visualizer/config_applier.py | apply_vis_mode_kwargs, build_gpu_push_extra_kwargs, _color_or_none | Per-mode keyword↔attribute mapping; caches the last applied settings snapshot for `_reset_visualizer_state()` and passes rainbow/ghost/bubble uniforms. Spectrum pushes mirrored/linear lane-strength maps through this layer instead of retired scalar lane controls. Blob/Bubble continuous energy routing uses the current mode-owned control lane contract (not persisted sidecar toggles), and shared glow uniforms are built from mode-specific osc/sine settings at GPU push time. |
...
| Tick Pipeline | widgets/spotify_visualizer/tick_pipeline.py | on_tick, process_heartbeat, dispatch_bubble_simulation, consume_engine_bars, push_gpu_frame, record_tick_perf, log_audio_latency_metrics | Main tick entry point, heartbeat detection, bubble dispatch, engine bar consumption, and GPU push. Also owns paused-idle mode flow safeguards (fresh-frame wait bypass where safe, paused transition progression, and worker-off idle rendering continuity for bubble/sine/oscilloscope). |
...
| Media Builders | ui/tabs/media/*.py | build_spectrum_ui(), build_oscilloscope_ui(), build_starfield_ui(), build_blob_ui(), build_helix_ui(), build_sine_wave_ui(), build_bubble_ui(), build_goo_ui() | Per-visualizer UI builders (extracted from widgets_tab_media.py, ~1400 LOC). Mirrored spectrum editor now preserves center→edge node semantics in UI mapping; osc/sine builders expose glow reactivity sliders. Goo builder is dev-gated behind `-devgoo`. |
| Goo Liquid Field | widgets/spotify_visualizer/goo_liquid_field.py | GooFieldState, GooSource, seed_goo_field(), solve_goo_field_step(), pack_sources_for_upload() | CPU-side edge-sourced metaball solver for Goo mode (~32 sources, 4 edges, band-assigned: bass/mid/high/overall). UI-thread dispatch from `tick_pipeline.dispatch_goo_field()`; renderer uploads packed vec4 array as `u_goo_sources`. |
| Goo Renderer | widgets/spotify_visualizer/renderers/goo.py | get_uniform_names(), upload_uniforms() | Uploads Goo mode uniforms including color swatches, outline/shadow/specular strength, void size, threshold, and the `u_goo_sources` vec4 array (matches `const int GOO_SOURCE_COUNT = 32` in goo.frag). |
| Goo Settings Binding | ui/tabs/media/goo_settings_binding.py | load_goo_mode_settings(), collect_goo_mode_settings() | Load/collect Goo tab settings (colors, outline, void, motion, ghost) through the standard `_config_*`/`_default_*` helpers on WidgetsTab. |
