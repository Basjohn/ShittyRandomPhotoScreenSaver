...
Current normal implementation work is **Phase F0.5 — retire hidden shadow-tuning authority, normalize canonical shadow settings, and complete Widgets → General controls before retained family ports**.
...
- **Phase F is active on current documentation basis `f049baed`; F0 is closed; F0.5 retires hidden shadow tuning + completes Widgets → General controls; F1 Clock follows after audit with per-family retire-on-GREEN cleanup thereafter.**
...
| Detailed family ports / Clock | `Docs/QtQuick_Migration/10_Widget_Family_Port_Decomposition.md`, `Docs/QtQuick_Migration/11_Clock_Analogue_Shadow_Contract.md` |
...
Its Quick port intentionally includes the analogue reference contract in
`Docs/QtQuick_Migration/11_Clock_Analogue_Shadow_Contract.md`, plus:
...
Source may still contain different kinds of migration reference, with different retirement timing:

- the old physical `DisplayWidget` / QRhiWidget / `GLCompositorWidget` stack remains until H cutover;
- an **unported ordinary family** may keep its QWidget pixels only while they are still needed as the
  visual/behavioral reference for that family;
- after a family is independently GREEN, its caller-proven old pixel presenter should be deleted before
  later families rather than carried to I;
- painter/QPixmap/effect machinery that exists only for already-retired presentation behavior should not
  be preserved as a fallback.

Git history becomes the historical reference after a family is retired. Real provider/network/cache/
transition recovery and the closed Visualizer failover contract are resilience behavior, not migration
fallback debris.
