...
| Steam Achievement Pulse resolver | `core/steam/achievement_pulse.py` | Pure cache/fixture selected-app and achievement-progress resolver that maps schema display names and the primary achieved-icon URL over internal achievement ids, retains the newest five unlocks plus previous/recent display names, and supports Most Recent, Recent #2-#5, and literal Custom app ID while preserving unavailable/private states |
| Steam Achievement Pulse cache bridge | `core/steam/achievement_pulse_cache.py` | Cache-first selected-app snapshot assembly, one-record recent-title lookup for Settings, and bounded successful-result-only recent/achievement-schema refresh work; successful equivalent responses freshen source records, shared source locks plus a short-lived profile/namespace/selection marker suppress immediate cross-card/display follower batches, unauthorized refreshes retain cache, and the module owns no threads, timers, UI, or credential decryption |
| Steam Abandonment resolver | `core/steam/abandonment_issues.py` | Pure verified-timestamp eligibility plus 15-minute floor, 26-week/sub-2-hour/cache-only-0-2-unlock preference tiers, all-unlocked demotion without completion claims, pinned/unavailable behavior, and cooldown-aware selection without provider or Qt dependencies |
| Steam Abandonment cache bridge | `core/steam/abandonment_cache.py` | Cache-first owned/recent resolution, at most 12 exact-path achievement hints prioritized current/pinned-first (no scan/request), 24-hour library/shared recent freshness, profile-shared policy-fingerprinted exposure/30-minute rotation state, cache-only advances, and cached-library Settings choices |
| Steam card visual components | `widgets/steam_components.py` | Provider-inert Steam view models, deterministic authored layout metrics, measured occupancy-aware field planning, paint fingerprints, and shared painter helpers that keep `Custom` as uniform scale instead of content authority; Achievement Pulse owns header-aligned game art, centered-below-art Unlocked geometry, a one-to-five unprefixed latest-unlock hierarchy, an optional 40px primary icon positioned after measured unlock text, colon-free compact capsules, and default-on all-field double headings/fitted values with `PREVIOUSLY`, independent capsule-font growth, RGBA styling, and exterior shadows |
...
| Steam Abandonment visual components | `widgets/steam_abandonment_components.py` | Provider-inert authored archival layout and renderer: large cover shelf, warm archive tab, title, optional stable 60/40 rediscovery message, reliable `LAST VISIT` stamp, optional ledger rows, Custom scaling, and dynamic-content opacity isolation |
| Steam Abandonment widget | `widgets/abandonment_issues_widget.py` | Production cache-first Abandonment overlay with worker-owned cache/provider/art preparation, profile-shared cache-only rotation, DPR-local paint assets, transition-safe manual refresh, and atomic fade commits of game/title/art |
| Steam settings shell | `ui/tabs/widgets_tab_steam.py` | Lazy public Steam Settings section with family master, nested card buckets, explicit OpenID/API-key controls, cache-only game choices, Achievement customization, and Abandonment session-floor/playtime/unlock/inactivity preferences, 30-minute rotation, Never Show, rediscovery-message visibility, art/accent/Guilt/field controls, and explicit worker-owned library refresh; only Journey/Friend buckets need `--devsteam` |
...
| Blob contour runtimes | `widgets/spotify_visualizer/blob_math.py` / `blob_pockets.py`; `widgets/spotify_visualizer/renderers/blob_unshaped_runtime.py` / `blob_shaper_runtime.py` | Blob-owned 128-sample CPU contour solvers: Mighty anchored grow/release fields and event pockets with contour-first response, plus Shaped authored-goal routing with independent idle, vocal, tendril, music, and true transient mutation beyond the goal; no shared-audio authority |
