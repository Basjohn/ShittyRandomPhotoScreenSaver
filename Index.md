...
| Vis Signal Policy | widgets/spotify_visualizer/config_applier.py | _resolve_continuous_energy_bands(), build_gpu_push_extra_kwargs() | Shared visualizer signal handoff. All modes use the canonical smoothed continuous-energy path by default; pre-AGC is only selected when `use_raw_energy` is explicitly enabled, and Blob must not grow a sidecar signal path. |
...
| Config Applier | widgets/spotify_visualizer/config_applier.py | apply_vis_mode_kwargs, build_gpu_push_extra_kwargs, _color_or_none | Per-mode keyword↔attribute mapping; caches the last applied settings snapshot for `_reset_visualizer_state()` and passes rainbow/ghost/bubble uniforms. Energy source selection reads `widget._use_raw_energy` — when True, uses `get_pre_agc_energy_bands()` for full dynamic range; when False (default), uses `get_energy_bands()` (smoothed post-AGC). Shared glow uniforms are built from mode-specific osc/sine settings at GPU push time. |
