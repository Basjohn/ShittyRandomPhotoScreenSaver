...
| Bubble simulation | `widgets/spotify_visualizer/bubble_simulation.py` | CPU-side Bubble particle simulation, two-lane Bubble sizing/gating (hero big lane plus small/medium lane), fast fixed-threshold sustained-loud movement/hold envelope kept separate from the soft transient lane, live size-envelope adaptation for existing bubbles, and per-frame big-lane diagnostics used by the authored Deep Sea regression bars |
