...
| Widget descriptor registry | `rendering/widget_descriptors.py` | Canonical factory-backed widget family metadata plus WidgetsTab section, runtime-capability, and stack-preview/settings-composition registry: stable widget ids, parent attributes, factory routing, startup-stage intent, inheritance kwargs, config injection such as Gmail shadow plumbing, settings-section order/label/builder ownership, service-backed/anchor-dependent capability flags, live-refresh handler ownership, canonical widget position-option/layout-edit capability metadata, future resize/reset capability flags, and descriptor-owned preview field reads for WidgetsTab stack/status composition |
