...
| Bar Computation | widgets/spotify_visualizer/bar_computation.py | fft_to_bars, compute_bars_from_samples, maybe_log_floor_state, get_zero_bars | DSP/FFT bar computation pipeline (inline, extracted from audio_worker); Mar 2026 retune adds micro-drop freeze, stronger large-drop release, dynamic-floor drop relief, faster normalization release on real dips, and (Mar 12) a median+hysteresis gate ahead of reactive smoothing to nuke <2% bar jitter while preserving big collapses |
