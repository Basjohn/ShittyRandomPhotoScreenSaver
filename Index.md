...
| Steam asset cache | `core/steam/assets.py` | Injected-fetcher Steam artwork/avatar/achievement-icon HTTPS host/signature validation, atomic cache writes, bounded local eviction, public app-art URL/local lookup helpers, and bucketed worker-side Abandonment desaturation |
...
| Steam Abandonment widget | `widgets/abandonment_issues_widget.py` | Production cache-first Abandonment overlay with worker-owned cache/provider/art preparation, profile-shared cache-only semantic rotation whose remaining interval survives rebuilds, selected public-art cache-miss hydration on the existing IO job when updates are allowed, forced non-repeating manual draws, shared transition-deferral retry for collided expiries/manual refresh, DPR-local paint assets, and atomic fade commits of game/title/art |
| Steam settings shell | `ui/tabs/widgets_tab_steam.py` | Lazy public Steam Settings section with family master, nested card buckets, explicit OpenID/API-key controls, cache-only game choices, Achievement customization, and Abandonment session-floor/playtime/unlock/inactivity preferences, 30-minute rotation, Never Show, rediscovery-message visibility, art/accent/Guilt/evidence-shelf controls, and explicit worker-owned library refresh; only Journey/Friend buckets need `--devsteam` |
