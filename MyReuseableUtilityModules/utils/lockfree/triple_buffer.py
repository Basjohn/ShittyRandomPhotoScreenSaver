...
class TripleBuffer(Generic[T]):
    def __init__(self):
        ...
        # Indices: write -> slot producer writes next;
        #          ready -> slot index with latest published value (or -1 if none)
        #          read  -> last consumed slot index (to avoid re-reading)
        self._write = 0
        self._ready = -1
        self._read = -1
    ...
    def publish(self, value: T) -> None:
        ...
        self._ready = i
        # Advance write to a different slot than ready and read
        nxt = (i + 1) % 3
        # Avoid pointing write to the same slot the consumer most recently read,
        # but in SPSC with overwrite semantics it's acceptable; still rotate.
        self._write = nxt
