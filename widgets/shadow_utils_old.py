...
from widgets.uncached_shadow_effect import UncachedDropShadowEffect
...
def apply_widget_shadow(
    ...
    *,
    ...
    Phase E: If shader shadows are enabled via WidgetManager, this function
    skips applying QGraphicsDropShadowEffect to avoid cache corruption.
    """
    ...
    # Phase E fix: Use UncachedDropShadowEffect to prevent cache corruption
    # on multi-monitor setups during context menu interactions
    if isinstance(existing_effect, (QGraphicsDropShadowEffect, UncachedDropShadowEffect)):
        effect = existing_effect
    else:
        effect = UncachedDropShadowEffect(widget)
        try:
...
class ShadowFadeProfile:
    ...
    def _start_shadow_fade(
        ...
        *,
        ...
        Phase E: If shader shadows are enabled via WidgetManager, skip the
        QGraphicsDropShadowEffect entirely to avoid cache corruption.
        """
        ...
        try:
            ...
            def _on_value_changed(value: float) -> None:
                ...
                try:
                    ...
                    pass
                # Notify WidgetManager for shader shadow fade sync (Phase E)
                try:
                    overlay_name = getattr(widget, "_overlay_name", None)
                    widget_manager = getattr(widget.parent(), "_widget_manager", None)
                    if overlay_name and widget_manager and hasattr(widget_manager, "update_widget_shadow_fade"):
                        widget_manager.update_widget_shadow_fade(overlay_name, f)
                except Exception:
                    pass
            ...
            def _on_finished() -> None:
                ...
                try:
                    ...
                    pass
                # Final notification to WidgetManager for shader shadow (Phase E)
                try:
                    overlay_name = getattr(widget, "_overlay_name", None)
                    widget_manager = getattr(widget.parent(), "_widget_manager", None)
                    if overlay_name and widget_manager and hasattr(widget_manager, "update_widget_shadow_fade"):
                        widget_manager.update_widget_shadow_fade(overlay_name, 1.0)
                except Exception:
                    pass
