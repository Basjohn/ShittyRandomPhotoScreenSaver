...
class GLCompositorBurnTransition(BaseTransition):
    ...
    direction:
        Burn direction — 0=L→R, 1=R→L, 2=T→B, 3=B→T, 4=TL→BR, 5=TR→BL.
    jaggedness:
    ...
    def __init__(
        ...
        self._direction: int = max(0, min(5, int(direction)))
    ...
    def _resolve_easing(self) -> EasingCurve:
        return resolve_easing(self._easing_str, EasingCurve.CUBIC_IN_OUT)
