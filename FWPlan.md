...
- [ ] **FW3 — Sphere — "Requires Much Higher Fidelity Assessment/Rework, keep 3D architecture work if ever retired unless it is completely superceeded - Consider Voxels?"**
  Current Sphere is dormant-by-default and therefore **not a migration-close blocker**. Preserve useful source-fenced immutable capture, lazy Settings, event-only inactive GPU retirement, static GPU-owned mesh/material plumbing and other reusable 3D substrate if the visual mode is retired; remove it only if a replacement completely supersedes it. The current liquid/fissure/material pass is not accepted visual fidelity. Future assessment should be willing to redesign the representation rather than incrementally polish it; voxel/volumetric approaches are explicitly worth assessing. Historical evidence remains in `Docs/Future_Work/Sphere_Visualizer_Decomposition.md` and `Docs/Future_Work/Visualizer_Edit_Geometry_And_Sphere_Materials.md`.
- [ ] **FW4 — Directional Pixel Accretion:** create a decomposition, then implement a deterministic instanced directional translation experiment with source underlay and exact endpoints.
- [ ] **FW5 — Glass Shatter / Exploding Tiles:** separate decompositions and isolated lazy implementations after higher-priority closure work; prove depth, deterministic launch and resource retirement.
