...
### 2026-09-05 CUSTOM terminalization / dead-Quick-root recovery

- `tests/test_qtquick_retained_lifecycle_integrity.py` — detailed retained-Quick lifecycle matrix across the current owners. It pins all-display/shared-owner terminalization even when coordinator and display cleanup both fail; healthy geometry Save and coherent cross-display Visualizer Save staying in-generation with no reload; layout-slot deferred Save remaining the explicit reconstruction boundary; corruption-triggered Save/Cancel ordering (finish first, exactly one repair request after); unexpected ordinary Qt-root death being one-shot while intentional retirement is not corruption; `DisplayScene` root loss counting as corruption only while admission remains open; and `DisplayManager` diagnostics remaining observational so one poisoned display snapshot cannot abort teardown. This complements `test_terminal_runtime_destruction`/barrier coverage rather than recreating retired lifecycle owners.
