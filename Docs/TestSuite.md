...
| `tests/test_qtquick_h_cutover.py` | **KEEP — MIGRATION PERMANENT / H** | H authority-cutover/cardinality/deletion bars, including a Quick-only DisplayManager source surface with no legacy presenter compatibility branch, one canonical manager-admitted visualizer owner, retained menu/double-click mode actions, hidden-boundary hard join, exact-once engine activation transaction, fresh-target reveal and engine-retirement lifecycle. |
