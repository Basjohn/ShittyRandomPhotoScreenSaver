...
| `tests/test_bubble_viewport_reflow.py` | **KEEP — MIGRATION PERMANENT / G4** | Baseline exact no-op plus wide/tall/shrink domain projection, authored-count invariance, no geometry-created tick, and canonical/wide/tall consume-once transient head/trail motion with radius isolation. |
