...
- `tests/test_steam_abandonment_issues.py`
  Steam Phase 7 Abandonment coverage: strict timestamp/title filtering, 15-minute eligibility, ordered 2-hour/2-unlock/26-week preference tiers, all-unlocked demotion without completion claims, bounded exact-path cached-achievement evidence with no provider work, exact UTC last-played formatting and unknown-date omission, cached achievement/latest-unlock/zero-unlock/archive-class shelf truth, all-eight-field adaptive ledger geometry, policy-change reranking, deterministic varied preference-biased draws, persisted non-sequential archive-position coverage, immediate-repeat exclusion, profile-shared cooldown/draw-counter rotation driven by the shared Steam refresh interval, exact five-minute remaining-duration/timer behavior, two-second due-boundary tolerance, persisted remaining cadence across widget rebuilds, forced non-repeating manual draws, shared single-shot transition-collision deferral, selected public-art cache-miss hydration with definitive-miss alternate-shape fallback versus transient/no-update no-fanout behavior, exact stable 60/40 rediscovery-copy buckets plus visibility control, source TTL/reuse/failure preservation, worker-prepared Guilt/art, authored rendering, cache-before-fade lifecycle, and sparse transition repaint bounds.
