...
- `tests/test_compositor_gpu_queries.py` and `tests/test_gl_timer_queries.py` — explicit heavy-profile gating, bounded one-in-eight passive draw-span sampling, transition labels, observation/support/sample/error accounting and no scheduling/cadence side effects;
