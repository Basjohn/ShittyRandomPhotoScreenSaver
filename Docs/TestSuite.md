...
- `tests/test_default_settings_editor.py`
  Authoritative Normal-base/MC-difference resolution, recursive future-setting discovery, compact MC serialization, generated valid-text hover descriptions for every string leaf, alpha-swatch/font delegate selection plus real-tree modal focus-transfer/cancel/accept/canonical-persistence behavior, privacy-filtered SST/JSON import including CUSTOM/layout-slot exclusion, MC-difference preservation across Normal import, standalone editor construction, dual-source transactional save/single-level undo behavior, and injected regeneration-failure rollback without touching real generated artifacts.
- `tests/test_regenerate_sst_defaults.py`
  Direct Normal/MC canonical SST generation: byte-identical repeat runs, deterministic non-operational metadata, full profile-builder parity, canonical MC differential classification, preserve-only Weather-coordinate exclusion, credential/private-field rejection, atomic output, and proof that generation cannot construct SettingsManager or mutate an installation settings sentinel.
...
- `tests/test_steam_profile_assets_events.py`
  Steam-family profile policy-state persistence, corrupt-state quarantine, app-art/schema-achievement-icon HTTPS host/signature validation and cache reuse, HTTP 404 classification plus bounded shape-aware app-art fallback ordering, fixture backend injection, and narrow non-secret `EventSystem` publication.
- `tests/test_steam_phase3_settings_descriptors.py`
  Steam descriptor/default/settings coverage: public Achievement Pulse and Abandonment Issues plus `--devsteam`-only Journey/Friend cards, true family-master gating, preserved choices, nested buckets, shared/card defaults, lazy behavior, safe connection hydration, signal-silent cache-only game labels/pickers, Achievement customization, Abandonment 15-minute floor plus 2-hour/2-unlock/26-week preferences, shared Steam refresh as the sole automatic-change cadence with conflicting legacy rotation values ignored/removed, Never Show/rediscovery-message/art/accent/Guilt/evidence-shelf round trips and adaptive default geometry, hidden prototype buckets, freshness settings, legacy reads, and descriptor-driven creation/reuse.
- `tests/test_steam_phase4_mock_visuals.py`
  Steam visual-system coverage: deterministic fixture view models, provider-inert painter helpers, content/paint fingerprints, DPR render variants, optically matched header/artwork strokes, shared top/right rails for Wide plus clamped 140/190 Square and 1.4-ratio Portrait cover crops with Portrait as the 140 x 196 default, widened centered-below-art Unlocked geometry with full high-count rendering in every artwork mode, portrait/card growth that preserves capsule gaps, measured-text-adjacent non-reflowing 40px primary-achievement icon geometry/paint, compact versus all-field double-capsule placement, centered doubled headings (`PREVIOUSLY` for Previous), independent capsule-font-driven capsule/card growth, colon-free compact labels, full-width value fitting, collision-free whole-rail growth, five-unlock prefix-free hierarchy, exterior shadows, long/unavailable states, and tight `Custom` scaling that preserves authored visible-field count and rails.
- `tests/test_steam_achievement_pulse.py`
  Steam Phase 5/6 Achievement Pulse coverage: pure selected-app resolution, up-to-five recent candidate ranking by newest positive unlock timestamp with stable recent-play fallback, matching Settings-label/Previous order, exact-path bounded cache loading, candidate-achievement refresh followed by selected-schema-only refresh, relevant-source freshness that ignores unrelated stale library age and accepts successful empty recents, ordered five-unlock schema labels, primary achieved-icon join with insecure/missing-icon fallback, persisted Custom app-ID settings, opaque cache snapshot/age handling, successful-response freshness updates, immediate multi-display follower suppression for unchanged payloads, cache-before-first-fade behavior, 5-minute runtime freshness threshold, manual-refresh routing, cached unauthorized connection-attention handling, literal unavailable/custom states, shared connect-required prompt geometry, stale connection info thresholding, and card click target emission.
- `tests/test_steam_abandonment_issues.py`
  Steam Phase 7 Abandonment coverage: strict timestamp/title filtering, 15-minute eligibility, ordered 2-hour/2-unlock/26-week preference tiers, all-unlocked demotion without completion claims, bounded exact-path cached-achievement ranking with no provider work, one selected-app-only achievement hydration for enabled shelves with canonical cache-key/24-hour reuse/stable identity/no fan-out, exact UTC last-played formatting and unknown-date omission, cached achievement/latest-unlock/zero-unlock/backlog-class shelf truth, privacy-safe active-shelf/evidence diagnostics, all-eight-field adaptive ledger geometry, policy-change reranking, deterministic varied preference-biased draws, persisted non-sequential backlog-rank coverage, immediate-repeat exclusion, profile-shared cooldown/draw-counter rotation driven by the shared Steam refresh interval, exact five-minute remaining-duration/timer behavior, two-second due-boundary tolerance, persisted remaining cadence across widget rebuilds, forced non-repeating manual draws, shared single-shot transition-collision deferral, selected public-art cache-miss hydration with canonical Portrait/legacy-square normalization plus definitive-miss alternate-shape fallback versus transient/no-update no-fanout behavior, exact stable 60/40 rediscovery-copy buckets plus visibility control, title/reminder shrink-floor rendering, source TTL/reuse/failure preservation, worker-prepared Guilt/art, authored rendering, cache-before-fade lifecycle, and sparse transition repaint bounds.
...
- `tests/test_custom_layout_manager.py`
  CUSTOM edit-session save/cancel behavior, global session activation across active display instances, first-class `custom` position persistence, saved authored-route reset behavior, late screen-binding recovery, size-payload persistence for clock/weather/media plus Reddit/Gmail font-driven resize families, stale Gmail size-payload rejection for settings-owned Text Balance while preserving the exact outer rect, live peer-snapping/clamp behavior, canonical rebuild-on-save/reset behavior, numbered-monitor cross-display ownership transfer, `ALL`-locked transfer blocking, deferred processed-image flush behavior, non-forced widget visibility during runtime custom-layout reapply, edit-shell reset-affordance placement, temporary per-display grid overlay lifecycle, `ThreadManager.single_shot(...)` ownership for coalesced/deferred global edit-shell restacks, visualizer shell capture via card-plus-overlay composition rather than whole-display grabs, visualizer-owned CUSTOM rect persistence including committed-rect replay authority over live mode-height policy, and media-owned CUSTOM persistence plus scale-contract reapply for secondary dependents like `spotify_volume`.
...
- `tests/test_clock_widget.py`
  Clock fade-sync parity, analogue circular background-card behavior, and exact whole-second hand angles that ignore callback microsecond jitter while advancing all hands from the existing shared one-second ticker.
...
- `tests/test_widgets_tab_general.py`
  User-facing General rename without internal-id churn, Appearance/Layout/Cache bucket composition and default expansion state, exact cache-family checkbox inventory, selection-gated clear action, confirmation scope, worker completion feedback, and Settings performance-cache snapshot invalidation routing.
...
- `tests/test_spotify_visualizer_widget.py`
  Secondary-stage startup ownership, manager/coordinator reveal routing, mode-specific paused reveal behavior (idle reveal for Oscilloscope/Bubble/Sine/Dev Curve versus fresh-engine wait for Spectrum), post-reset stale-frame blocking, parent deadline coordination, activation/reset runtime contracts, live audio block-size capture rebinding, lifecycle-aware latency diagnostics, Bubble dispatch hot-path and visible-lane/authored-motion/perf guards with disproven U-07 proxy/current-feel signatures explicitly retired, profile-window worker timing that rejects sustained cost without treating one host-scheduler stall as deterministic Bubble work, live big-size edit authority, seeded Preset 1 vs Preset 9 comparison, Spectrum GPU extras reuse, solid-Spectrum coherent-zero hold and delayed-frame catch-up bounds, authored Spectrum first-visible/startup-parity oracles, hot mode-switch/preset-cycle parity, and architecture-split engine-resolution coverage.
...
- `tests/test_gmail_widget.py`
  Gmail cache/fallback behavior, empty-fetch preservation of valid displayed mail, empty-state header-safe layout, shared transition-aware refresh deferral, shared manual-refresh short-circuiting, timer cleanup, grouping formatting, reserve-first sender/subject ratio bounds, retired subject-character-limit behavior, dimensionless CUSTOM ratio scaling, and stable-content caching rules.
...
- `tests/test_rss_behavior.py`
  RSS facade/current pipeline behavior: mocked multi-feed JSON ingestion from non-Reddit fixture URLs, wallpaper-sized image validation/download caching, queue rotation across RSS/local pools, duplicate-history protection, and explicit fail-fast guards against accidental live Reddit URL usage.
...
- `tests/test_visualizer_card_geometry.py`
  Supported visualizer outer card geometry parity: mode/preset-owned preferred dimensions, shrink-to-base behavior for strip-like modes, and media-relative placement ownership separate from stencil math.
...
- Retired visualizer behavior tests are not retained as creative or runtime oracles. Safe migration, production absence, and unchanged supported-mode bars are the contract.
...
- Do not revive retired visualizer creative/runtime behavior tests; retain only forward-migration and absence guards.
