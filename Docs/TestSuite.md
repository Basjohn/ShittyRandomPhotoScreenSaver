...
- `tests/test_media_command_ingress.py`, `tests/test_media_artwork_layout.py`, `tests/test_media_display_update.py`, `tests/test_media_transition_deferral.py`, and `tests/test_media_widget_runtime_methods.py` — process-wide Qt/native/raw media-route deduplication with once-only visualizer wake, authoritative in-flight artwork generations, worker QImage decode, pre-diff generation promotion, key-owned GUI pixmap replacement, newest-only all-display transition deferral, static transition-time feedback with one-shot-only cleanup even beside animated feedback, playback-state repaint without redundant Qt layout setters, DPR-sized header-logo reuse, and split feedback/presentation lifecycle telemetry;
