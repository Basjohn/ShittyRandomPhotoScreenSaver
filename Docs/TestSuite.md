...
- `tests/test_steam_phase3_settings_descriptors.py`
  Steam descriptor/default/settings coverage: public Achievement Pulse and Abandonment Issues plus `--devsteam`-only Journey/Friend cards, true family-master gating, preserved choices, nested buckets, shared/card defaults, lazy behavior, safe connection hydration, signal-silent cache-only game labels/pickers, Achievement customization, Abandonment 15-minute floor plus 2-hour/2-unlock/26-week preferences, 30-minute rotation, Never Show/rediscovery-message/art/accent/Guilt/evidence-shelf round trips and adaptive default geometry, hidden prototype buckets, freshness settings, legacy reads, and descriptor-driven creation/reuse.
