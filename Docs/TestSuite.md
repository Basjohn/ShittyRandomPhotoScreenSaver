...
**Most recent operator evidence, 2026-09-20:** the expanded three-action undo/third-generation owner gate passed **6/6 on Windows**; the focused Reddit Restore/spacing and offline Achievement Pulse/Weather owner gate passed **7/7 on Windows**; the operator also accepted the rendered Reddit Restore and title/age clearance. The earlier Reddit/Reddit2 owner Save/fresh-generation/second Edit gate passed 2/2. The revised owner lifecycle gate includes Achievement Pulse/Weather second Save and third fresh generation, accepted in the six-case run. The bounded three-action Edit undo is accepted in that run; real user keyboard and physical end-to-end acceptance remain distinct. The earlier Media absent-Edit-delegate test fixture was corrected to exercise real model-driven side placement; its focused Windows run passed. That acceptance does not automatically validate later changed families.

**Current acceptance gates:** Reddit/Reddit2/Gmail semantic column rails passed Windows Qt and physical checks. Media's external-volume automatic side placement is an accepted semantic contract and its model-driven opposite-axis Qt gate passed; do not bring back a fixed-side test. Achievement Pulse artwork and Weather location selected painted-target geometry passed focused Qt and physical alignment checks, and the later drag check was smooth after removing reactive non-bindable transform-list reads. Digital Clock compact rendering/drag passed physical acceptance. The subsequent cumulative Windows native gate passed **85/85**, including the Clock stable-delegate and System Stats three-role tests, and the operator physically accepted the System Stats three-role editor. These completed gates are accepted for the unchanged baseline; the later selected-child snap-guide focused Windows gate also **passed 25/25 in 2.12s** (operator report). Do not request repeat physical acceptance of these unchanged family behaviours, nor present local source guards as a runtime performance benchmark. The full maintained destination profile is pending for this tree.
...
### Followed-game news source and adaptive-layout admission

`tests/test_steam_games_followed_source.py`, `tests/test_steam_games_followed_projection.py`, `tests/test_steam_games_followed_g0.py`, `tests/test_steam_games_followed_news_g0.py`, `tests/test_steam_backend.py`, `tests/test_steam_request_policy.py` and `tests/test_steam_profile_assets_events.py` cover the opt-in linked followed-set provenance, request budget, cache privacy, generation drop, Steam-owned exponential backoff after 429/denial, safe source-to-display projection and X/Y layout capacity/hysteresis. These tests do **not** certify article/browser redirects, image loading, shared Steam runtime admission, real networking beyond the two-app G0 probe or an end-to-end OSD smoke. `tests/test_steam_games_followed_g2_staging_contract.py` guards the deliberately unadmitted retained Qt model/QML and presentation-neutral grouped child roles; `tests/test_qtquick_games_you_follow_staging.py` now tests actual Qt/QML fixed-slot identity, X/Y capacity, edit records, hide/reveal and retirement. The **new G2 native Qt gate is pending Windows execution**: passing source-only checks cannot certify the real QML scene or future host/Save/reopen. The user reports the preceding 55-case G1 source gate passed. Tests are not a source refresh scheduler.
