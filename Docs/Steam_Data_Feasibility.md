...
- `core/steam/abandonment_issues.py`, `abandonment_cache.py`, `widgets/steam_abandonment_components.py`, and `widgets/abandonment_issues_widget.py` own the second production card: strict timestamp provenance, short-start/age/cache-only-achievement ranking tiers, profile-shared policy-aware cooldown rotation, worker-prepared optional desaturation, a distinct archival ledger composition, and sparse manager-owned content crossfades.
