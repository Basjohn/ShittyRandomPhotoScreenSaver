...
| Per-app achievements | `ISteamUserStats/GetPlayerAchievements/v1` + `GetSchemaForGame/v2` | Conditional | achievement list, unlock state, schema totals/names | Requires user key, profile id, app id; per-app availability may vary | Achievement Pulse can proceed with unavailable/private branches |
...
- May proceed after synthetic fixtures cover recent games, owned games, achievement list, schema totals, private/unavailable, and empty achievement responses.
