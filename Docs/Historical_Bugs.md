...
### Recent Resolutions
1. [R-29 — 2026-06-30 — Reddit Periodic Refresh Due Reset On Settings/Runtime Rebuild (Resolved In Code, Runtime Validation Pending)](#R-29)
...
- **Additional 2026-06-30 evidence:** `--cache` logs proved the scheduler itself did fire (`reddit2` at `11:13:26`, `reddit` at `11:27:56`, `reddit2` at `11:28:26`), but a later manual/provider attempt hit Reddit `429` at `11:34:10`. The old blocked-error path then touched the ordinary JSON cache timestamp, making failed/blocked fetches look like fresh content on startup/settings return.
...
- **Fix extension:** blocked `403`/`429` handling now refreshes only the Reddit service cooldown gate and leaves the content JSON mtime unchanged. Periodic scheduling respects the persisted blocked gate/due horizon, so restarting during cooldown cannot immediately re-probe a stale cache.
- **Fix extension:** the old timed cache-growth reveal was retired because Reddit fetches the full `25`-post candidate window used by the widget. The widget now caches that candidate window and immediately displays the configured visible count. Manual refresh uses a shorter `3min` gate for fresh-cache, periodic-due, and blocked-cooldown skip decisions so the spiral is useful without changing the automatic `15min` sparse cadence.
- **Fix extension:** RSS remains the default provider, but `core/reddit_post_provider.py` now includes selectable HTML plus a designed HTML fallback from selected-provider failure. It tries `www.reddit.com/r/<subreddit>/` and then `old.reddit.com/r/<subreddit>/` through the same stable persona and request-slot seams, and only successful post rows can update the content cache.
- **Bars:** `tests/test_reddit_widget.py` covers fresh-cache startup skip, stale primary due firing, stale secondary periodic stagger, stale secondary startup pacing, preserved due surviving widget rebuild, blocked errors preserving cache mtime truth, blocked cooldown delaying periodic fetch, manual-refresh skip paths including the `3min` blocked-cooldown window, fixed candidate-window fetch/display split, and padded Reddit age labels. `tests/test_reddit_post_provider.py` covers standard/old Reddit HTML parsing and composite fallback from primary failure. `tests/test_reddit_progressive_loading.py` guards the retired growth seam so staged reveal helpers do not return.
- **Runtime validation target:** future `--cache` logs should show fresh caches skipping with `cache_fresh`, stale paired startup caches producing `Startup stale refresh paced ... delay_s=...`, both `reddit`/`reddit2` eventually logging `[CACHE][REDDIT] Periodic refresh fired`, manual skips using the shorter window, HTML fallback success/failure as loud provider evidence, and blocked fetches logging `[CACHE][REDDIT][RATE_LIMIT] ... cache timestamp unchanged`. Provider/API blocks remain a separate track if the scheduler fires correctly but Reddit still rejects requests.
