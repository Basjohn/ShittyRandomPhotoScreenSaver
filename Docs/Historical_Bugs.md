...
- [R-91 — Worker Processes Outlived A Crashed UI Process](Historical_Bugs/R-91_Worker_Processes_Outlived_Crashed_UI.md) — **FIXED IN CODE / AWAITING VALIDATION**. Workers detect parent death; installed End-task check open.
- [R-90 — Core Audio Endpoint Released Twice (ctypes.cast Shared A COM Pointer)](Historical_Bugs/R-90_Core_Audio_Endpoint_Double_Release.md) — **FIXED IN CODE / AWAITING VALIDATION**. `QueryInterface`, never `ctypes.cast`, between COM interfaces.
