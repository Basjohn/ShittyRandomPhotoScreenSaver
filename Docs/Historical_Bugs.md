...
- The remaining narrower seam after that fix was tick-owned overlay over-requesting (`set_state` around `90-98fps` on a 60Hz owner) plus occasional high `dt_max`. The follow-up correction caps actual overlay `update()` requests to the owning display target while still accepting every visualizer payload, preserving current-good visualizer reactivity without asking a child GL surface to paint frames the display cannot present. Runtime validation is still required before closing this branch of the perf family.
