...
The repository-wide prerequisites are now complete: Gmail OAuth callback ownership is manager-bound, the supported Oscilloscope/Bubble baseline is locked without visual retuning, Normal/MC SSTs are canonical and byte-reproducible, and the failed Blob mode is retired behind a migration-only boundary recorded as R-46. `Current_Plan.md` therefore resumes with the compiled paused-visualizer pressure bar and then the Steam runtime bars below. This preserves an authoritative defaults source and trustworthy supported-mode baseline before spending the longest compiled-runtime budget, without demoting any Steam source, privacy, visual, Custom, lifecycle, or performance gate.
