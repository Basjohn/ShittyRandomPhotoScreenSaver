...
- Purpose: protect Achievement Pulse unlock-recency candidate ordering and selected-schema request bounds, Abandonment's shared-Steam-refresh-driven rebuild-proof/forced non-sequential semantic rotation cadence plus bounded selected-art hydration/fallback boundary, source provenance, successful-only cache authority, cross-card/display request reuse, cache-before-fade lifecycle, worker-prepared assets, sparse content transitions, settings/default parity, and deterministic authored geometry.
