...
Parser 1.14 treats a directory named `logs` as the live sidecar root: it reads only
...
update-request, `paintGL`, CPU paint-duration, and state-to-paint rates. Parser 1.14
...
Parser 1.14 additionally parses `[PERF][GL COMPOSITOR][GPU]` windows and groups their
...
Parser 1.14 preserves visualizer transition name/elapsed/idle-age/latency-trigger fields
...
select one run rather than the whole `evidence_chest` parent. Parser 1.14 joins copied
...
Parser 1.14 also groups lifecycle barriers by reason and tracked resource samples by
