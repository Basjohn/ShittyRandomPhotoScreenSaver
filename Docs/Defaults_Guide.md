...
| `widgets.spotify_visualizer.ghosting_enabled` | `True` | Legacy global settings fallback only. Modern authored preset payloads must use per-mode ghost keys instead of shipping this global mirror. |
...
**Per-mode technical schema (Mar 2026):** Every mode owns an explicit set of living technical keys (`<mode>_bar_count`, `<mode>_dynamic_floor`, `<mode>_manual_floor`, `<mode>_adaptive_sensitivity`, `<mode>_sensitivity`, `<mode>_audio_block_size`, `<mode>_dynamic_range_enabled`, `<mode>_agc_strength`, `<mode>_input_gain`, plus the mode-appropriate transient controls). Defaults for Spectrum/Bubble/Blob/Sine/Osc are defined in `defaults.py` under `widgets.spotify_visualizer` and seed `<mode>_manual_floor = 0.12`. `technical_controls.py` now uses one shared metadata registry to build/load/save these controls, while special transient-mix keys such as `spectrum_lane_transient_mix` and `bubble_transient_mix_vocal` stay as direct keys rather than doubled prefixed variants. The global `bar_count`/`dynamic_floor` fallbacks remain for legacy SST snapshots but should not be edited going forward. Retired compat keys `energy_boost` / `use_raw_energy` are no longer part of the persisted settings/defaults schema.
...
**AGC / legacy technical controls (Mar 2026):** `agc_strength` remains part of the living visualizer schema. `energy_boost` and `use_raw_energy` have been retired from persisted settings/defaults/preset authoring; runtime still has internal support for derived AGC boost and optional debug-time raw-energy reads, but those are not first-class authored settings anymore.

**Blob continuous-energy policy (Mar 2026):** Blob stays on the same shared continuous-energy source of truth as the other visualizer modes: smoothed/post-AGC energy by default. Do not "fix" future Blob feel issues by inventing a Blob-only sidecar signal path or extra Blob-local settings storage outside `widgets.spotify_visualizer`; investigate tuning/runtime shaping while keeping the shared signal contract intact.
...
**Preset hygiene:** Use `tools/visualizer_preset_repair.py` to refresh curated preset JSON/SST payloads whenever defaults change. The tool now canonicalizes through the shared visualizer mode-payload normalizer before rebuilding authored JSON, so alias values are preserved while authored schema debt is removed. That means curated/custom preset authoring should stay mode-owned and modern:
- no `custom_preset_backup` blocks
- no retired compat keys such as `*_energy_boost` / `*_use_raw_energy`
- no global ghost mirrors (`ghosting_enabled` / `ghost_alpha` / `ghost_decay`) in authored preset payloads
- no retired Oscilloscope alias `osc_sensitivity` in authored preset payloads; use `osc_line_amplitude`
