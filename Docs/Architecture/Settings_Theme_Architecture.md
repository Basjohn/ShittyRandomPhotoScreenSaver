...
`ui/settings_theme_spec.py` owns the compiled `DEFAULT_DARK_SETTINGS_THEME` and schema-v6 semantic vocabulary.
...
`ui/settings_theme_io.py` admits a file whole or rejects it whole. Current parsing is schema-v6-only after the explicit `ui.settings_theme_input_compat` boundary has promoted an exact supported schema-v5 user file. Unknown/missing roles, wrong format/schema or invalid values do not partially merge into a theme. Compiled Default Dark remains the unconditional no-file/failure fallback.

Schema v6 owns `about.art.liquid`, the Settings-only colour authority for the explicitly masked liquid/background field in `images/Logo.png` and `images/Shoogle300W.png`. The companion masks preserve source shading/alpha and leave bubbles, text, outlines, fish shading and drop shadows untouched. Every shipped theme seeds `about.art.liquid` from that theme's primary accent (`chrome.outer_border` RGB at full alpha), so Nocturne Split resolves to its pink-red primary while blue/silver/etc. themes use their own primary colour. The role remains explicit/editable in Theme Foundry; this is value alignment, not a second runtime inheritance mechanism. Supported schema-v5 user themes are admitted only by `ui.settings_theme_input_compat`: the v5 colour-role set must be exact, then `about.art.liquid` is seeded from `chrome.outer_border` RGB at full alpha and the payload becomes v6 before strict current parsing. No arbitrary missing/unknown role is default-merged.
...
- expose `Off`, `Acrylic` and `Glass` as supported schema-v6 modes;
