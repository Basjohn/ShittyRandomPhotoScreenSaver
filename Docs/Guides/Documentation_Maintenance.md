...
| `Docs/Architecture/Settings_Theme_Architecture.md` | permanent Settings theme / Acrylic / Glass / Foundry contract |
...
A file may remain physically under `Docs/Future_Work/` after its implementation closes if moving it would create noisy reference churn. In that case its header must say **RETIRED / HISTORICAL REFERENCE** and name any genuinely surviving future tail. Folder name alone never admits work. Each such file's own header records that lifecycle classification so a later agent does not execute completed decompositions as plans.
...
Keep documentation organized by current role rather than by the implementation phase that happened to create it:

- `Docs/Architecture/` — durable architecture and subsystem ownership;
- `Docs/Guardrails/` — binding invariants and preflight contracts;
- `Docs/Guides/` — authoring/change procedures and maintained how-to material;
- `Docs/Reference/` — current lookup tables, feasibility notes and harness routing;
- `Docs/Future_Work/` — genuinely pending or operator-activated implementation plans;
- `Docs/Historical_Bugs/` — permanent regression and failed-method evidence;
- `Docs/Fossils/` — superseded decompositions retained only for archaeology;
- `Docs/audits/` — point-in-time reconciliation/closure evidence.

Keep only the small routing authorities at `Docs/` root (`00_PROJECT_OVERVIEW.md`, `Contracts.md`, `Guardrails.md`, `Historical_Bugs.md`, `TestSuite.md`) plus non-prose generated evidence that has a stable path contract. `Index.md` lives at repository root and must be regenerated/reconciled whenever documents move.

A GODZIP document move is represented as the file at its new path plus a manifest `move_to_deleteme` entry for the old path. This preserves Foundry recovery semantics and avoids leaving duplicate live authorities behind. Historical/fossil/audit bodies are not rewritten solely because a current document moved; their old path language is evidence-scoped.
...
QtQuick docs, `Docs/Guides/10_WIDGET_GUIDELINES.md` when ordinary-family patterns changed, feature plans naming concrete owners,
...
For Settings theme/backdrop ownership changes, also inspect `Docs/Architecture/Settings_Theme_Architecture.md`, Theme Foundry,
