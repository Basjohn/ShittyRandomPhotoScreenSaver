...
- [ ] Prefer retained wireframe vector icons over emoji for new navigation/chrome: QPainter or the already proven retained vector owner, inheriting the parent control's existing semantic resting/selected/disabled colours and the product-wide bright-white active-hover cue where that control is a clickable border/text target. Preserve the existing shadow and interaction owners, no icon-specific timer or polling, and no duplicate theme-state cache. Replace older emoji glyphs only in a bounded, test-backed related pass.
