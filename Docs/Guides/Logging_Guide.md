...
The capture is **not** a process-level stderr redirect. Raw non-Qt native fd-2 writes remain outside ordinary release-file capture; Diagnostic `faulthandler` is a separate direct fatal plane. See `Docs/Guides/Qt_QML_Observability.md` before adding an OS-level stderr tee.
