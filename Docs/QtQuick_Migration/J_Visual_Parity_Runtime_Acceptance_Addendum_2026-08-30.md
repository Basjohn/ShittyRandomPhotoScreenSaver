...
> **2026-09-03 STATUS / AUTHORITY FENCE.** This file is retained as detailed J parity provenance, not a second live checklist. `Current_Plan.md` owns surviving physical acceptance. In particular, the later Widget Theme cleanup retired Media/Gmail/Reddit/Steam family Header Fill/Border/Text swatches and the Media `Header Appearance` bucket. Shared `header.*` Widget Theme semantics now own branded-header colours; `Widgets -> General -> Style Overrides -> Header Fill` is the one ordinary GUI header-fill authoring surface, and Media `Show Header Pill` lives in normal `Appearance`. Any older “family header swatch” wording below is superseded by the corrected cells in this file and current theme contracts.
...
- [~] Slice 6 shadow/theme-role refinement: artwork/transport/app-volume hard duplicate shadows are superseded by small cached `RectangularShadow`s using the same Python-resolved global direction vector with `1.20x` / `1.15x` / `1.05x` displacement respectively. Their blur is intentionally bounded to `25%` of global blur, clamped `2..6 px`. App-volume outline/fill remain genuinely Media-specific authoring. **Later supersession:** Media header Fill/Border/Text family swatches were retired after Widget Theme semantics matured; branded-header colours now resolve through shared `header.*` authority. This checkpoint's physical label is historical; current acceptance is in `Current_Plan.md`.
- [~] Slice 7 header closure: Media consumes the same `BrandedHeader.qml` primitive as the other branded families, including ALL-CAPS intrinsic-width text, logo shadow, retained text shadow, scale-aware border and a cached direction-aware extension shadow using the Media transport-bar shadow profile. **Later supersession:** Header Text is now shared Widget Theme semantic authority rather than a Media-local swatch. Current physical status belongs to `Current_Plan.md`.
...
- [~] **Slice 9 external-volume closure / Media parity lock.** Media's canonical app-volume control is the scene-local **external right accessory rail**, default ON whenever Media exposes app-volume capability. `OverlayWidget.rightAccessoryExtent/rightAccessoryContent` keeps that rail in the same retained root/lifecycle/CUSTOM transform while the ordinary card reclaims the accepted full content width; the display-level card shadow deliberately covers the card only, not the accessory lane. The rail's Track/Fill/Outline and Seek Bar's Track/Fill/Shadow/Glow remain genuinely Media-specific controls in collapsed `Volume Control` / `Seek Bar` buckets. **Later supersession:** the separate `Header Appearance` colour bucket was removed; `Show Header Pill` moved back into `Appearance`, and Header Fill/Border/Text resolve through shared Widget Theme semantics. The accepted gray volume fallback remains unless a real `media.volume.fill` / shared accent theme role overrides it. Later J/J+ work must not move volume back inside the card, steal reclaimed width, reset accepted Media alignment/shadows/strokes/fades, or recreate family-local header palette authority.
...
- [~] Slice 7 shared branded-header normalization: Media, Gmail, Reddit, Achievement Pulse and Abandonment Issues consume one `BrandedHeader.qml` contract: 25 px logo / 8 px gap / 20 px total horizontal padding / 36 px minimum height / 9 px radius / family-invariant 16.4 pt bold ALL-CAPS label. Header text is never elided; the pill widens to full logo + label width. Fill/Border/Text now resolve through shared Widget Theme `header.*` semantics rather than family-local settings, while retained text/logo/header shadows and scale-aware stroke compensation remain shared presentation behavior. Gmail/Reddit `header_logo_px_adjust` remains retired. Current physical status belongs to `Current_Plan.md`.
...
- [~] Slice 8 Widget Theme semantic-role projection: one shared sparse resolver owns Header Fill/Border/Text, Media transport/mute/volume/progress roles, Gmail action/separator roles, Reddit/Weather/Clock separators, Steam panels/tooltips/artwork/gradients/metrics, and the retained Context Menu palette. Default-valued compatibility fields remain implicit Inherit; genuinely changed **surviving specialized** family swatches may remain explicit overrides. Branded-header family swatches are retired. `local.*` semantic values are runtime context and never serialize. Context Menu Default Dark was reconciled to accepted retained QML pixels before semantic replacement. Do not re-hardcode migrated colours during parity work.
...
- [~] Slice 9 Settings organization was later simplified after shared header authority landed: Media uses `Provider & Layout`, `Appearance`, `Artwork`, `Transport Controls`, `Seek Bar`, and `Volume Control`. `Show Header Pill` belongs in `Appearance`; the one-control `Header Appearance` bucket and family header colour swatches are retired. Semantic grouping remains UI presentation only and must not become a second theme/state/geometry owner. Do not mechanically expose every sparse theme role.
