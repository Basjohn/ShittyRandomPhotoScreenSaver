...
- [ ] Validate Slices 1–7 against the actual widget at ordinary scale plus enlarged and shrunk CUSTOM sizes: confirm the smaller/narrower artwork no longer crowds or clips the seek region, embedded Spotify video bars are gone, rounded corners contain all source pixels, ordinary album art is not falsely cropped, metadata/seek +2 px and mute inset remain correct, Title/Artist/Album casing looks natural on mixed/all-caps provider data, long metadata shrinks before clipping without wrapping, each optional line hides/reappears immediately and survives Settings reload/layout-slot use, artwork/transport/volume shadows all follow at least two non-SE global directions with the requested relative offsets and only slight blur, alpha in the header/volume swatches is honored, the header extension shadow keeps opposite-edge coverage, and the four scale-aware framing strokes remain legible without exceeding the +/-1 px visible-thickness contract.
- [~] **Media parity lock.** On successful Slice-7 physical validation, mark Media visual parity complete. From that point J/J+ may not rework the accepted header geometry/casing/shadows, metadata offsets/fitting, artwork geometry/crop/mask, seek geometry, transport strip, mute geometry, volume geometry/shadows or scale-aware strokes as collateral to another family's parity pass. The one named post-parity geometry enhancement is an optional **external app-volume rail, default ON**, with reclaimed card-content width; treat it as a deliberate feature slice. Shared artwork-fade gentleness and semantic theme-role projection are cross-family infrastructure polish, not permission to reset Media geometry.
...
- [ ] **J/J+ protected authored baseline.** Family-specific parity work may correct each widget's outer padding, content spacing, row geometry and placement of the shared header as a whole, but it must not fork/rewrite the accepted `BrandedHeader` internals, restore Gmail/Reddit logo-size exceptions, reduce refresh opacity from 70%, move refresh/edit-X off the shared row, remove the accepted logo/text/header shadows, or undo already-accepted Media geometry/shadow/stroke choices unless the operator opens a specifically named regression. Parity is not authority to revert newer authored improvements merely because the pre-migration screenshot differed.
- [ ] Future line-width rollout: apply the shared visible baseline +/-1 px, minimum-1-px stroke helper incrementally to ordinary non-Visualizer separators/borders where thickness is decorative. Do not blanket-convert semantic/content geometry, and do not regress already-migrated uses. Visualizer remains exempt.
- [ ] Future Widget Theme role projection: branded-header Fill/Border/Text, app-volume Fill/Track/Outline, transport surface/border/separators, mute surface/border/icon, decorative separators/outlines, secondary panels and gradients should resolve through one semantic Widget Theme palette. Prefer inheritance/fallback roles over exposing every literal as a top-level swatch; explicit per-widget overrides remain possible without becoming the default GUI surface.
- [ ] Future fade polish: tune `ArtworkFadeImage.qml` centrally for gentler Media + Steam artwork transitions, then add event-driven Media metadata opacity transitions on track identity changes if validation shows no freshness/latency regression. No recurring animation timer or provider polling may be introduced.
