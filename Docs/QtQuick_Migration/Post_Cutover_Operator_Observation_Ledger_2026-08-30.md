...
| [x] | O-051 | **H — CLOSED** | Middle-click same-mode Visualizer preset cycling was missing. | H8 restored and operator physically validated wraparound/Custom round-trip/recreation. Preserve one visualizer owner/runtime/pacer and narrow persistence. |
...
| [ ] | O-055 | **J geometry/parity** | Weather scaling once emitted repeated `preferredContentHeight` binding-loop warnings from `WeatherPresentation.qml`. | Source-localize the preferred-size/layout cycle and require repeated resize + Save/recreation with zero loop warnings. Keep in J unless new evidence proves functional/committed geometry corruption; H is already closed. |
| [x] | O-056 | **H — CLOSED** | One image change had bare-flashed the destination instead of delivering a transition. | R7 transactional admission/prefetch repair accepted; subsequent heavy/manual/natural transition runs showed no recurring bare/black flash and timer/manual timing populations overlap. Preserve R-65. |
| [ ] | O-057 | **J optional device-edge refinement** | A transition can intermittently leave a one-device-pixel shared-edge overshoot. | H accepts the bounded <=1px residual because native telemetry proves mixed-DPR rounding (`2561` window over `2560` monitor) while R-63 keeps black/stale flash at 0. Any J refinement must derive device-space geometry generically across DPR/resolution/topology; never trade `black=0` for exact cover. |
| [x] | O-058 | **H — CLOSED** | Media steady-state polling was replaced by native GSMTC event observation. | Repeated H runs accepted native observation with prompt reactions and clean `stale_rejected=0`, `missed=0`, `degraded=False` lifecycle summaries. Preserve slow reconciliation/watchdog only; no silent fast-poll/process-probe fallback may return. |
...
H is closed. Any still-open row is explicitly J/preservation/residual work and must not silently reopen H. J cannot close from unit tests alone where the defect is physical pixels, clipping, interaction feel or black flash.

`Current_Plan.md` owns active sequencing; this ledger preserves operator-visible evidence and J acceptance obligations.
