...
| [ ] | O-015 | **H active (H5c)** | Bubble magnitude and Play/Pause are dramatically better; elasticity still flickers and the last physical run found transient motion imperceptible. | The 2.72-2.76x radius projection loss is fixed and physically accepted. Continuous visual-rate interpolation plus stable tracked-bubble evidence now awaits elasticity validation. A separate consume-once transient correction reuses the existing decaying stream-burst state, caps drift lift at `0.18`, and proves extra same-body stream/drift motion-stage displacement with identical radius/pulse. Re-measure both visual results; preserve resizing, source/cadence/radius authority and do not global-gain tune. |
...
| State | ID | Phase | Observation | Durable handling / close condition |
|---|---|---|---|---|
| [x] | O-037 | J | OS cursor + retained halo/cursor shape can appear as double cursors. | **R6 IMPLEMENTED / AWAITING PHYSICAL CONFIRMATION:** the retained fake pointer is removed. Halo presentation is now the window's native `QCursor`; context-menu/semantic suppression selects one ordinary arrow cursor and normal non-interaction mode remains blank. Pointer movement no longer moves a QML item or republishes semantic state. |
