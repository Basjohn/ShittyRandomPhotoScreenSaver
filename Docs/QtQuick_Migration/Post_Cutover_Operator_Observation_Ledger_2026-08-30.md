...
| State | ID | Phase | Observation | Durable handling / close condition |
|---|---|---|---|---|
| [x] | O-037 | J | OS cursor + retained halo/cursor shape can appear as double cursors. | **IMPLEMENTED / AWAITING PHYSICAL CONFIRMATION:** Halo admission now explicitly blanks the native cursor; intentional Halo suppression (including retained context menu) exposes one ordinary cursor. High-rate pointer motion/inactivity remain in the retained QML scene; same-scene auxiliary ownership remains. |
