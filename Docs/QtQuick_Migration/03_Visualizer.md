...
coordinates at the render seam. Stream/drift movement is authored in renderer-content units, so each nonbaseline axis is
projected once into that expanded world and trail smear is evaluated in content space; otherwise a doubled axis silently
halves visible movement. This is spatial projection, not a gain/control/cadence retune, and canonical `1x1` remains exact.
Bubble also preserves its historically card-height-normalized authored radius; dividing radius by domain height is a
reactivity loss, not viewport reflow. Collision/spawn geometry applies the inverse `domain_h` mapping to that radius and its
collision-only gap/correction distances so visible contact remains coherent without an aspect-aware physics rewrite.
