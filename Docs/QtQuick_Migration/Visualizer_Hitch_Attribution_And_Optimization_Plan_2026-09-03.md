...
**Fix (implemented + validated in-situ):** `RuntimeGCPolicy.freeze_stable_generation()` runs once from a one-shot ~45 s after `start()` (a one-shot, not a poller), moving the stable long-lived set to the permanent generation; `gc.unfreeze()` runs on stop. It does not disable GC or hide leaks (post-freeze allocations collect normally).
