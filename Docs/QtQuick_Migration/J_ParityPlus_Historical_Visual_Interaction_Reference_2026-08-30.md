...
- [ ] Widget Theme card palette is a global/default baseline: explicit `widgets.<family>.card.*` values win for that family, while the Context Menu consumes Widget Theme palette directly. Per-family swatch edits do not create Widget Theme Custom; baseline/theme edits do.
