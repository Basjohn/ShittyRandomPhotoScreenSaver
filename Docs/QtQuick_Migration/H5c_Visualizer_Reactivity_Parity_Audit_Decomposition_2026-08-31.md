...
- [x] Production-path canonical/wide/tall A/B proves a doubled viewport axis previously halved renderer-visible stream/drift and trail travel even though raw world-step diagnostics looked unchanged.
- [x] Project stream/drift movement once onto each nonbaseline domain axis and solve nonbaseline trail smear in renderer-content coordinates. Keep canonical `1x1`, consume-once delivery, transient envelope, authored controls, pulse/radius sequence, cadence and gain exact.
- [x] Focused post-projection Bubble/runtime gate passes `116/116`, the maintained target-isolated H profile passes `84/84`, and an independent read-only review is GREEN; the physical viewport/transient recheck remains part of the next operator run.
...
- [ ] **2026-09-01 extreme-head correction falsified — archived as R-69:** the first Quick-only `head_radial_scale` capped all Bubble head radii past ~1.75x authored height and physically destroyed visible reactivity as CUSTOM height/width diverged. This contradicts the binding radius contract above: radius is already renderer-facing and remains a fraction of actual card height. Remove that global multiplier; retain only the requested smooth 0..+1 authored-pixel extreme-size outline bonus. Restoring this contract may make the extreme-tall full-expansion head physically very large again; accept that temporarily rather than flattening the entire response range. If that tail remains oversized after the contract is restored, target only the proven upper expansion tail rather than scaling the entire radius sequence. R-69 preserves the rejected method and why it failed. **AWAITING PHYSICAL VALIDATION:** tall/wide CUSTOM reactivity should match near-canonical feel.
- [ ] **2026-09-01 product decision / scaling correction:** operator confirms the old architecture also lacked useful Bubble Ghost fading and explicitly chooses a slow afterimage as the intended contract. Retained Quick reuses the existing three-sample Bubble smear history and existing Ghost shader loop; no second pass/history owner/timer/cadence is added. The first implementation was physically rejected on extreme CUSTOM aspect ratios because it reused the R4 ripple-wake `u_trail_axis_scale` after BubbleSimulation had already normalized history into renderer-content coordinates, applying a second `1 / viewport_extent` suppression to Ghost displacement. Ghost now consumes normalized history directly, while the accepted R4/R5 ripple wake stays compact and independent. The fade curve is modestly gentler. **AWAITING PHYSICAL VALIDATION:** canonical, wide and tall Ghost displacement/fade without excess wake clutter or reaction-latency regression.
