...
- [x] stream/drift transient motion correction is implemented at the existing owner: consume-once kick/snare/vocal strength immediately lifts the existing decaying stream-burst state, and transient drift drive is capped at `0.18`; no authored setting, source gain, radius, cadence or clock changes.
- [x] a same-body event/no-event oracle proves extra short-window stream and drift motion-stage displacement with identical pulse and rendered-radius sequences; bounded B6-B8 scalars expose the path.
- [ ] operator re-measure must confirm that stream/drift transient response is perceptible but not overdriven. The new motion-step scalars are pre-collision contributions, not final trajectory distance.

Classification: source/config/cadence and long Play transport delay are exonerated. The retained-old first observation is real but too brief to explain the full physical ramp. Final nonbaseline radius normalization was the magnitude defect and is physically validated as dramatically better. Removing the hard hold improved Play/Pause but did not close elasticity because the remaining two-rate threshold still toggled under a moving target. The display-only correction rate now varies continuously, with stable per-bubble evidence added at existing cadence. Transient motion is now restored through the existing decaying state with exact radius/pulse isolation. No gain, cadence, timer, contraction or latest-state contract changed; physical elasticity and transient-motion validation remain open.
