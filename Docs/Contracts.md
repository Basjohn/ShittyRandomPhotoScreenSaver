...
| Reddit overlay rendering / runtime behavior | `widgets/reddit_widget.py` | service-widget runtime, helper bridge | One Reddit card/runtime contract, including Reddit-local cache reuse, shared startup-attempt gating, per-cache-key periodic due scheduling that survives widget rebuilds, and width-fixed CUSTOM vertical content-fit participation for descriptor-approved height persistence. |
