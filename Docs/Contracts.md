...
| Contract Family | Canonical Owner | Related Files | Goal |
|---|---|---|---|
| Canonical defaults | `core/settings/default_settings.py`, `core/settings/default_profile_overrides.py`, `core/settings/defaults.py` | `tools/default_settings_editor.py`, generated JSON/SST artifacts | One resolved source of default truth: base, shared Normal differences, then MC-only differences. The editor discovers leaves recursively and writes only compact overrides. |
...
| Steam descriptor/settings/visual skeleton | `rendering/widget_descriptors.py`, `rendering/widget_factories.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py`, `ui/tabs/widgets_tab_steam.py` | Steam cards/settings, future Steam widget work | Achievement Pulse and the lazy Steam Settings section are active without a flag. `--devsteam` exposes only Steam Journey (`steam_progress` compatibility key), Abandonment Issues, and Friend Pulse descriptors/factories/buckets. Opening Steam may read non-secret encrypted-storage availability plus one recent-games cache record on shared IO for choice labels; constructors/paint helpers still cannot decrypt credentials, enumerate caches/assets, contact Steam, or own private polling timers. Achievement Pulse preloads opaque-keyed cache through ThreadManager before its first coordinated fade and schedules a bounded refresh only after fade. The shared freshness window is 5-240 minutes, default 10. |
| Achievement Pulse selection/view model | `core/steam/achievement_pulse.py`, `core/steam/achievement_pulse_cache.py`, `core/steam/assets.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py` | Achievement Pulse card, Steam cache/fixture bars | Selected-app resolution is pure cache/fixture logic for Most Recent, Recent #2-#5, and literal Custom app ID. Cached recent names decorate Settings labels without changing stable mode data. The model retains up to five newest unlocks, maps schema display names, and carries the second recent game for Previous. Wide artwork cover-crops the public header capsule and matches the header's top/right silhouette and stroke. Square artwork cover-crops Steam's portrait capsule at 140-190 authored pixels (default 140) and keeps Unlocked centered below it. Compact capsules use colon-free uppercase left labels and right values. Measured doubled fields reserve a same-column lower value capsule and center the upper heading; Previous becomes `PREVIOUSLY`, while other labels keep their wording. Alpha fill/border, exterior shadows, whole-rail growth, and uniform `Custom` scaling remain authoritative. |
