...
| Contract Family | Canonical Owner | Related Files | Goal |
|---|---|---|---|
| Canonical defaults | `core/settings/default_settings.py`, `core/settings/default_profile_overrides.py`, `core/settings/defaults.py` | `tools/default_settings_editor.py`, `tools/regenerate_sst_defaults.py`, generated JSON/SST artifacts | One resolved source of default truth: authoritative Normal base plus MC-only differences. The editor recursively discovers leaves, writes Normal into the base, keeps MC compact, offers alpha swatches/font controls/valid-text guidance and privacy-filtered SST/JSON import, and transactionally regenerates artifacts. SST generation always uses explicit temporary storage and cannot touch installed profile JSON; imports exclude active geometry/layout slots. A successful Foundry save becomes the contract unless proven harmful by reproducible runtime/safety evidence. |
...
| Steam source/backend/cache contract | `Docs/Steam_Data_Feasibility.md`, `core/steam/backend.py`, `core/steam/cache.py`, `core/steam/models.py`, `core/steam/achievement_pulse_cache.py` | Achievement Pulse runtime, Steam provider tests, Steam cache tests | Steam endpoints are source-classified before UI use, client-key requests use `api.steampowered.com`, publisher-only endpoints are barred client-side, backend tests use fake openers/fixtures, cache records preserve fetched timestamps, and failed/private/invalid responses cannot freshen versioned cache. Byte-equivalent successful Achievement Pulse payloads retain the existing record while a short-lived profile/cache-namespace/selection success marker suppresses immediate multi-display follower requests, preventing disk/repaint and duplicate-provider churn. Achievement Pulse cache/refresh helpers own no UI, timer, or thread scheduling. |
...
| Steam descriptor/settings/visual skeleton | `rendering/widget_descriptors.py`, `rendering/widget_factories.py`, `rendering/widget_setup_all.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py`, `ui/tabs/widgets_tab_steam.py` | Steam cards/settings, future Steam widget work | Achievement Pulse and the lazy Steam Settings section are active without a flag. `--devsteam` exposes only Steam Journey (`steam_progress` compatibility key), Abandonment Issues, and Friend Pulse descriptors/factories/buckets. The Steam family master gates factory creation and expected-overlay truth while preserving card choices; subordinate settings use Layout/Appearance/Content buckets. Opening Steam may read non-secret encrypted-storage availability plus one recent-games cache record on shared IO for choice labels; constructors/paint helpers still cannot decrypt credentials, enumerate caches/assets, contact Steam, or own private polling timers. Achievement Pulse preloads opaque-keyed cache through ThreadManager before its first coordinated fade and schedules a bounded refresh only after fade. The shared freshness window is 5-240 minutes, default 10. |
| Achievement Pulse selection/view model | `core/steam/achievement_pulse.py`, `core/steam/achievement_pulse_cache.py`, `core/steam/assets.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py` | Achievement Pulse card, Steam cache/fixture bars | Selected-app resolution is pure cache/fixture logic for Most Recent, Recent #2-#5, and literal Custom app ID. Cached recent names decorate Settings labels without changing stable mode data. The model retains up to five newest unlocks, maps schema display names, joins the primary unlock to its HTTPS schema icon, and carries the second recent game for Previous. The optional 40px framed icon sits immediately after measured primary-unlock text without moving title/art/Unlocked/capsule geometry and disappears on missing/invalid assets. Wide/square artwork, centered Unlocked, compact or default-on all-field double capsules, independent capsule-font/card growth, alpha styling, exterior shadows, whole-rail growth, and uniform `Custom` scaling remain authoritative. |
