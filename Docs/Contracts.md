...
| Abandonment Issues selection/view model | `core/steam/abandonment_issues.py`, `core/steam/abandonment_cache.py`, `core/steam/assets.py`, `widgets/steam_abandonment_components.py`, `widgets/abandonment_issues_widget.py` | Abandonment card, source/cache/visual/lifecycle bars | Eligibility requires owned status, title, configurable 15-minute floor, positive non-future verified `rtime_last_played`, 12-week age, recent exclusion, and Never Show exclusion. Ranking prefers 26-week-old sub-2-hour starts and opportunistically 0-2 unlocked achievements from at most 12 exact cached snapshots; unknown is neutral and no sweep/request occurs. All-unlocked is only a demotion signal. Policy changes rerank once; 30-minute rotation remains profile-shared/cache-only. Default-on rediscovery copy is deterministic per game, assigns `Long Forgotten` 60 of 100 stable buckets and each of ten authored alternatives four buckets, and can be hidden independently of functional state messaging. Owned freshness is 24 hours, recent follows shared settings, Guilt art is worker-prepared, and sparse shared-animation fades preserve stable chrome. |
