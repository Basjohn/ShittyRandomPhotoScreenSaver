...
| Reddit overlay rendering / runtime behavior | `widgets/reddit_widget.py` | service-widget runtime, helper bridge | One Reddit card/runtime contract, including Reddit-local candidate-window cache reuse, immediate configured visible-count display, fresh-cache startup skip, paced paired-stale startup attempts, per-cache-key periodic due scheduling that survives widget rebuilds, `3min` manual refresh gating, loud blocked/rate-limit failure handling, and width-fixed CUSTOM vertical content-fit participation for descriptor-approved height persistence. |
