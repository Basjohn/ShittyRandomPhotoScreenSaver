...
| Gmail cache preparation/commit | `core/gmail/gmail_preparation.py`, `widgets/gmail_widget.py` | `Docs/audits/SRPSS_Architecture_Roadmap/08_CPU_THREADING_AND_WORKLOAD_PLAN.md` | Startup stat/read/JSON and newest-wins atomic persistence on shared IO; request/revision-gated Qt commit; GUI static QPixmap prepared outside paint with exact size/DPR/revision identity; no synchronous service fallback |
