...
coordinate with sibling Layout/Appearance/Content buckets without collapsing their parent card; Feeds nests
one level deeper (News/Custom group -> card -> leaf), one open bucket per level; Visualizer Custom
...
- The FEEDS family uses four fixed CUSTOM identities and six fixed NEWS category identities, all admitted through the same adapter, generated runtime descriptor, retained presentation and one card-parameterized Settings builder (`feeds_custom{N}_*` / `feeds_news_<category>_*` controls, one CUSTOM-lock scope per card). Custom 1 uses the shared `BrandedHeader` and established semantic header palette; publisher metadata is a separate small per-slot optional subtitle below the pill. Header alignment is the one widget-wide semantic orientation intent, so header/corner, refresh rail, Grid order, List artwork/text/age rails and overflow alignment mirror together without mirroring image pixels. Its configured-name monogram (with a small collision ordinal when live cards share an initial) is rasterized once per semantic config through a cached antialiased QPainter wireframe/segment renderer and supplied to QML as an immutable local data URI; there is no emoji/font-glyph, Canvas repaint, timer or file-cache owner. Feed documents are fetched through explicit timeout/byte/redirect limits (a website address resolves to its verified feed by bounded standards-based discovery, never per-site rules, stored beside the endpoint's last-good state), normalized from RSS/Atom/JSON Feed/JF2/h-feed documents into one model, and stored as atomic versioned last-good snapshots with persisted failure backoff. Image absence never invalidates an item. CUSTOM acquisition identity is the endpoint fingerprint, so identical active endpoints share one cache/source/cadence while each card keeps independent presentation; changing endpoints cannot display the prior endpoint's cache. One generation-scoped Feed family owner owns earliest-due scheduling, active leases and source-specific cancellation, on one family cadence (`widgets.feeds.refresh_minutes`, never per card): a source due within a quarter interval joins the current wake-up unless it is in failure backoff; cache-only admission does not construct network transport until refresh is actually due. Optional artwork is one event-admitted, bounded local-cache batch on that existing source lane, not a second cadence. Feed-contained media candidates are deduplicated across stories by canonical URL identity and normalized image content before presentation; List and Grid admit local artwork per stable story identity while an image-less story uses its authored text-only row/card, and Compact remains text-only. Retained QML owns no timer, network or remote image source, and renders every feed string as `Text.PlainText` (a publisher's `<canvas>` stays text; no markup can load an image). Max Items is a ceiling and presentation paints only complete rows/cells that fit. Custom 1 participates in the shared independent X/Y `content_extent` Edit contract and the shared stable CUSTOM-child geometry contract (`header`, `refresh`, `articles` group, repeated `artwork`, `overflow`); volatile article IDs are never child persistence keys. The one `artwork` role supplies common freeform geometry to repeated List/Grid image paint and its Edit proxy targets the first actually admitted local image, avoiding per-story persistence churn. Hover/reflow/child editing perform no feed/image I/O. `--feeds` owns focused diagnostic INFO/DEBUG in `screensaver_feeds.log`. A NEWS card holds one ordinary lease per selected publisher on the same family owner (stable provider cache identity with endpoint migration, never URL identity) and publishes a pure newest-first merge: exact story-URL deduplication only, publisher attribution on every row, and a failed publisher never blanks the healthy ones. Artwork is warmed only for the leading stories an active card can show (its item limit). See `Docs/Reference/Feeds.md`.
