...
| Gmail cache preparation/commit | `core/gmail/gmail_preparation.py`, `widgets/gmail_widget.py` | `Docs/audits/SRPSS_Architecture_Roadmap/08_CPU_THREADING_AND_WORKLOAD_PLAN.md` | Startup stat/read/JSON and atomic persistence on shared IO; immutable metadata publication; request/revision-gated Qt-visible commit on GUI; no synchronous service fallback |
