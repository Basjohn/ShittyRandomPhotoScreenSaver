...
| Steam descriptor/settings/visual skeleton | `rendering/widget_descriptors.py`, `rendering/widget_factories.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py`, `ui/tabs/widgets_tab_steam.py` | Steam cards/settings, future Steam widget work | Achievement Pulse and the lazy Steam Settings section are active without a flag. `--devsteam` exposes only Steam Progress, Abandonment Issues, and Friend Pulse descriptors/factories/buckets. Opening Steam may read only non-secret encrypted-storage availability so saved connection status rehydrates, while constructors/paint helpers still cannot decrypt credentials, scan caches/assets, contact Steam, or own private polling timers. Achievement Pulse preloads opaque-keyed cache through ThreadManager before its first coordinated fade and schedules a bounded refresh only after fade. The shared freshness window is 5-240 minutes, default 10. |
| Achievement Pulse selection/view model | `core/steam/achievement_pulse.py`, `core/steam/achievement_pulse_cache.py`, `core/steam/assets.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py` | Achievement Pulse card, Steam cache/fixture bars | Selected-app resolution is pure cache/fixture logic for Most Recent, Recent #2-#5, and literal Custom app ID. It retains up to five newest unlocks, maps schema display names, and carries the second recent game for the optional Previous capsule. Wide mode caches the public header capsule and cover crop. Square mode caches Steam's portrait library capsule, center-cover-crops it at a clamped 140-190 authored pixels (default 180), aligns it to the same header/right rails as wide art, and keeps Unlocked centered below it. Latest names have no generated prefixes. Supporting capsules use uppercase split label/value alignment, alpha-capable fill/border settings, strictly exterior bottom/right shadows, and an occupancy-aware optional double layout that reserves a directly aligned fitted-value capsule only when measured compact text would elide. Authored canvases grow by whole capsule rails when required; `Custom` remains uniform scale rather than content reflow. Source remains optional but defaults off; Previous defaults on. Private/unavailable/no-achievement states remain literal. |
