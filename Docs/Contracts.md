...
| Steam source/backend/cache contract | `Docs/Steam_Data_Feasibility.md`, `core/steam/backend.py`, `core/steam/cache.py`, `core/steam/models.py`, `core/steam/achievement_pulse_cache.py` | Achievement Pulse runtime, Steam provider tests, Steam cache tests | Steam endpoints are source-classified before UI use, publisher-only endpoints are barred client-side, backend tests use fake openers/fixtures, cache records preserve fetched timestamps, and failed/private/invalid responses cannot freshen versioned cache. Byte-equivalent successful Achievement Pulse payloads also retain the existing record, preventing cache/repaint churn. Achievement Pulse cache/refresh helpers own no UI, timer, or thread scheduling. |
...
| Steam descriptor/settings/visual skeleton | `rendering/widget_descriptors.py`, `rendering/widget_factories.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py`, `ui/tabs/widgets_tab_steam.py` | Steam cards/settings, future Steam widget work | Four Steam cards are independent dev-gated descriptor/factory/runtime/Custom participants. The lazy Steam Settings shell and all constructors/paint helpers must not decrypt credentials, scan caches/assets, contact Steam, or own timers. Achievement Pulse may load opaque-keyed cache and schedule its one startup refresh only after activation/fade through ThreadManager. `Custom` uniformly scales authored card elements and never decides visible-field count, rails, or content availability. Enabled cards without connection/cache show the shared Steam connect prompt; stale cached cards can show the default-on info affordance only after one day. |
| Achievement Pulse selection/view model | `core/steam/achievement_pulse.py`, `core/steam/achievement_pulse_cache.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py` | Achievement Pulse card, Steam cache/fixture bars | Selected-app resolution is pure cache/fixture logic for Most Recent, Recent #2-#5, and literal Custom app ID. Cache resolves after activation; automatic refresh respects startup freshness and explicit manual refresh bypasses only freshness, never in-flight dedupe/backoff. Private/unavailable/no-achievement states remain literal card states; no substitute game or provider call is allowed from constructors or paint. |
