...
| Steam descriptor/settings/visual skeleton | `rendering/widget_descriptors.py`, `rendering/widget_factories.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py`, `ui/tabs/widgets_tab_steam.py` | Steam cards/settings, future Steam widget work | Four Steam cards are independent dev-gated descriptor/factory/runtime/Custom participants. General Settings remains lazy; opening the Steam section may read only non-secret encrypted-storage availability so saved connection status rehydrates, while constructors/paint helpers still cannot decrypt credentials, scan caches/assets, contact Steam, or own private polling timers. Achievement Pulse preloads opaque-keyed cache through ThreadManager before its first coordinated fade and schedules a bounded refresh only after fade. The shared freshness window is 5-240 minutes, default 10. `Custom` uniformly scales authored card elements and never decides visible-field count, rails, or content availability. |
| Achievement Pulse selection/view model | `core/steam/achievement_pulse.py`, `core/steam/achievement_pulse_cache.py`, `core/steam/assets.py`, `widgets/steam_components.py`, `widgets/steam_card_widget.py` | Achievement Pulse card, Steam cache/fixture bars | Selected-app resolution is pure cache/fixture logic for Most Recent, Recent #2-#5, and literal Custom app ID. It retains up to three newest unlocks and maps successful schema records from internal ids to display names. Public app headers are validated/cached off the UI thread, DPR-aware smooth cover crops are cached for paint, and GUI preferences own font family/size, artwork visibility/shape, latest-unlock count, and supporting fields. Wide art aligns to the header, square art to the game title, data blocks stay bottom-anchored, and the redundant selection status is hidden by default. Private/unavailable/no-achievement states remain literal; no substitute game or provider call is allowed from constructors or paint. |
