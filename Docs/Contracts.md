...
| Visualizer cadence/presentation/repaint | visualizer tick owner plus display presentation | `Docs/Presentation_Change_Preflight.md` | Logical integration always first; `AdaptiveTimerStrategy` disqualified as a presentation source in any scope (R-61/R-62); a pacing source that degrades under load is disqualified; state-to-paint latency is an acceptance metric |
