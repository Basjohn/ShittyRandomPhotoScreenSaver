...
Status: implementation pending. Live sequence: `FWPlan.md`.
...
Elastic is a deterministic bounded arrival overshoot/rebound from canonical time, with exact zero modifier at
completion. Both image ownership and sampling derive from the same displaced coordinate. Overshoot past arrival
must retain full destination coverage and clamp sampling at the departing edge rather than wrap an opposite-image
strip across the screen. No unowned/black background branch, CPU integration, second clock or resources.
...
- [ ] Add frozen style resolution and the canonical default; reject invalid authored style values explicitly.
- [ ] Implement Elastic and focused dense endpoint/coverage/overshoot/uniform/resource tests.
- [ ] Implement Wobble/Flex only with equivalent deterministic coverage and endpoint proof.
- [ ] Expose proven options on lazy Slide detail, preserving choices through load/save/recreation and unbuilt saves.
- [ ] Run real GL oracle and inspect representative captures; update contracts, then narrow commit + push.
