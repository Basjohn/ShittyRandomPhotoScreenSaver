...
- **Media:** main artwork, volume bar and seek bar. Media playback controls are admitted only as one grouped `transport_controls` role. The role's edit/collision target covers the complete occupied controls band, including the system-mute control when present, so visible controls cannot sit outside the geometry/collision contract. Never persist or resize individual buttons.
...
9. **Pre-rollout editor-polish gate implemented in source, pending Windows/physical proof:** placement-capable roles default to all four resize corners; selected-parent sibling/parent alignment guides use light hysteretic snapping; descriptor-declared left/right alignment flip lives inside the existing child payload; parent close/Restore chrome fades from child gesture edges; the child layer has an explicit finite reveal; and CUSTOM owns pointer semantics so normal widget/runtime click actions cannot fire beneath Edit. Older size-only Achievement/Friend roles keep their historical anchor handle until their later placement migration because they cannot yet consume X/Y translation.
10. Only after the complete pre-rollout editor-polish gate is green, admit each widget's shared `BrandedHeader` as a role with strong semantic corner stickiness/hysteresis and prove that role through Save/Cancel/Restore/slots/parent resize.
11. Only after the shared header proof is green, roll the placement/layout-block primitive into Achievement Pulse Name/Achievement List/grouped Pills/Shelves, Friend Pulse and other dense widgets.
...
13. Extend to Clock analogue grouped roles and Weather only after their family-specific semantics are proven to fit the shared primitive without new ownership.
