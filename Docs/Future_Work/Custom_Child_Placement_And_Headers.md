...
**Widget-scoped Child Collision preference:** a widget with more than one declared editable child may expose the standard circular **Child Collision** Settings checkbox. Default is on. Turning it off disables only editable-child peer collision admission for that widget: children may overlap and move/resize through one another directly. Parent-bound clipping/containment, fixed family obstacles, snapping, sibling/parent alignment guides, semantic header anchors and all Edit ownership rules remain active. The preference belongs to the widget's normal Settings mapping (`child_collision_enabled`), never `child_geometry`, so Save/Cancel/Restore/slots do not rewrite it. An active CUSTOM session may consume Settings changes through the existing event-driven Settings notification; no timer, polling or render-cadence check is permitted. A future family with multiple editable roles must add the canonical default and standard Settings checkbox as part of its rollout slice rather than inventing a family-local collision switch.
...
6. **Collision preference:** any widget with multiple editable roles uses the shared `child_collision_enabled` Settings contract, default-on and circular-checkbox surfaced. Off disables editable-peer collision only; fixed obstacles, snapping/guides and parent containment stay active. It must not enter `child_geometry` or create a family-local persistence path.
