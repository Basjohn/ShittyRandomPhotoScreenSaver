...
**2026-09-14 status — COMPLETE (operator-accepted).** Production dependencies were severed without transplanting the monolith: `ui/settings_theme.py` and `ui/system_tray.py` no longer load/reference the file, the color-picker wrapper explicitly owns the only live legacy subsettings chrome found by caller proof, and the retirement contract is GREEN in the maintained destination profile. The Windows/PySide file-absent visual/lifecycle matrix passed and **`themes/dark.qss` has been physically deleted from the repository**; the application is accepted running without it. **Binding rule (permanent):** never recreate or restore the legacy file, introduce a replacement monolithic Settings QSS, or add a fallback stylesheet loader — even when the asset is absent. This document is now a historical execution record; the standing rule lives in `Current_Plan.md` (Standing guardrails) and `Docs/TestSuite.md` §6.3.
