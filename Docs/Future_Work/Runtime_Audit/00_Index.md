...
| PW-02 | **Confirmed** by fault injection: Media refresh (play/pause truth) and transport commands queue behind four stalled network tasks in the FIFO IO pool; the WinRT-affinity-lane fix is rejected (couples to teardown); Media-only lane awaits operator approval | P1 | R3 | Medium | fault-injection test |
| VZ-01 | `[~]` While paused, the BeatEngine synthesized a 256-sample sine waveform every tick for every mode; now only when Oscilloscope is active (paused tick 150.6 → 36.4 µs for other modes) — physical paused/edge check open | P2 | R2 | Medium | measured |
