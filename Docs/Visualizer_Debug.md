...
- **Reactivity smoothness guardrail (Mar 2026):** Sine’s renderer-local assist path now uses a quick-attack / slower-release smoothing pass in `renderers/sine_wave.py`. This was added to stop the old `subtle -> subtle -> oversized snap -> instant collapse` behavior while keeping beat pickup fast. Guardrails:
  - Do not try to fix this class of jerk by only changing AGC/dynamic-floor defaults; the confirmed root cause was renderer-local assist snapback.
  - Treat authored motion (`sine_wave_effect`, `sine_micro_wobble`) and reactive motion separately when tuning presets; high authored motion can still make quiet passages feel busier than loud ones.
  - If the issue resurfaces, inspect `[SPOTIFY_VIS][SINE][ASSIST]` logs first and compare raw vs smoothed assist values before changing presets or scheduler weights.
  - Regression coverage lives in `tests/test_transient_per_mode_integration.py::TestSineSchedulerBeatAssist` and `tests/test_transient_per_mode_integration.py::test_reactivity_spikes_rise_quickly_but_release_smoothly`.
