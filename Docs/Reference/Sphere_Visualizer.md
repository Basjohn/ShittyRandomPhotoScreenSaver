...
**Independent acoustic-floor controls:** The fragment Energy Floor and particle Energy Floor are Sphere-only, pre-AGC presence thresholds, evaluated after the existing typed/onset candidate qualifications. The fragment floor defaults to `0.000`, applying no extra admission filter; the particle floor defaults to `0.075`, the already-accepted authoring/typed-force floor. Both controls use `0.001` steps across `0.000..1.000` and have separate persisted `sphere_fragment_energy_floor` / `sphere_particle_energy_floor` keys in canonical defaults, Custom, and curated Sphere preset snapshots. Older user-authored preset payloads missing the keys receive these same existing-behaviour defaults. The particle slider cannot defeat the separate real-silence presence safeguard or existing hysteretic gate; it does not tune population, velocity, or in-flight cohorts. Fragment threshold changes neither tracer admission nor particle admission or packet amplitude. Do not change the shared event scheduler, pre-AGC spectrum, renderer, or other visualizer modes to implement these controls.
