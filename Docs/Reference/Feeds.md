...
`project_feed()` is pure and receives only a snapshot, display mode, limits and optionally already-validated local artwork sources. It never downloads or exposes remote URLs as render sources. Feed image URLs are only artwork candidates. Its optional `visible_item_capacity` parameter applies the Grid all-or-none rule to the actual visible row prefix, not items hidden behind `+N MORE`; retained QML supplies that capacity on real geometry changes without source fetching or per-image publication. Omitting it retains the strict full-requested-row check.

List permits sparse optional thumbnails because each row can collapse its image lane independently. Compact is text-only. Grid is generation-coherent: if every visible row does not have a validated local artwork source, every visible grid row receives an empty image source and the generation uses a coherent text-grid presentation. When the visible set is complete, all images may appear together. Before warming, artwork admission compares candidates across the accepted document: a media URL repeated by multiple distinct items is treated as feed/site chrome rather than article identity, while per-item candidates retain their existing quality ranking. This prevents one large generic feed hero from being stamped onto every article without adding article-page scraping or another network owner.

List and Grid article surfaces expose the same event-driven click affordance as other interactive cards: pointing cursor plus a semantic hover outline only while the existing interaction gate admits the action. Hover performs no provider/cache work and owns no timer.
...
The Feeds cache is explicit under the canonical application cache root (`cache/feeds`). User-facing cache maintenance may clear this last-good data; it does not clear CUSTOM URLs/names or any settings/credential material. Feed cache records are bounded by schema and file size. The F3 `FeedArtworkCache` is invoked by Custom 1’s existing shared source-owner IO lane below `cache/feeds/artwork/`. It uses URL-hash local filenames, verified normalized PNGs, at most four new image attempts per admitted batch, 256-file/128-MiB worker-write eviction and protection for published images of currently observed source generations. The `core/feeds/artwork_transport.py` callback pins a publicly vetted DNS answer before connecting, validates every redirect, retains HTTPS hostname verification and bounds response bytes/time/cancellation. Accepted article text publishes cache-first; one event-admitted optional artwork batch uses a shared eight-second budget and publishes a retained local-file image generation atomically. An unchanged source document does not trigger repeated missing-image requests on every normal source refresh. The runtime has no new image timer, separate downloader or GUI-thread file validation; multi-source simultaneous eviction and stalled OS DNS retirement remain explicit future activation gates.
...
The current runtime admits only `feeds_custom_1`, now with event-driven optional local artwork. The remaining fixed CUSTOM IDs and all NEWS IDs are non-runtime until their own gates. Before Custom 2–4 are enabled, add source-specific in-flight cancellation/pruning so an unused source inside a still-live multi-source family generation cannot finish needless work.
