...
### `themes/dark.qss` retirement — CLOSED (operator-accepted 2026-09-14)

Production Settings/tray dependency severed (permanent narrow owners + semantic `SettingsThemeSpec` QSS; ThemeSpec-backed tray `QMenu`; color-picker wrapper owns the residual legacy subsettings chrome), the retirement contract is GREEN in the maintained profile, the Windows/PySide file-absent visual/lifecycle matrix passed, and **`themes/dark.qss` is physically deleted** from the repository. Binding rule (do not reopen): never restore the file, add a fallback stylesheet loader, introduce a replacement monolithic Settings QSS, or copy old palette literals into Python. Historical record: `Docs/Future_Work/Settings_Dark_QSS_Retirement.md`, `Docs/TestSuite.md` §0.16/§6.3.
