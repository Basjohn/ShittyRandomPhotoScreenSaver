...
### `themes/dark.qss` retirement — runtime dependency removed; physical asset deletion pending

`themes/dark.qss` remains legacy source/package debris, not Settings Theme palette authority. The 2026-09-14 bounded audit severed the production Settings/tray loader dependency without transplanting the monolith: Settings root structure is generated by permanent narrow owners plus semantic ThemeSpec QSS, the tray has a ThemeSpec-backed `QMenu` renderer, and the color-picker wrapper owns the live legacy subsettings chrome, generic disabled labels now use the existing `text.disabled` semantic role, and the tiny root structural base retains palette-free dialog-button-box ordering/margin. The Qt-free retirement contract is **6/6 PASS** and production Python contains no legacy path reference.

This row stays open only for the intended Windows/PySide **Needs run** absence matrix and final physical deletion of the repository/build asset. GODZIP Foundry intentionally excludes `themes/`, so missing bytes in a checkpoint are still not proof that the real repository file is gone. Execution authority is `Current_Plan.md` section 3 and `Docs/Future_Work/Settings_Dark_QSS_Retirement.md`. Do not restore a fallback loader, copy old palette literals into Python, add a parallel theme role merely to preserve dead selectors, or disturb native AccentPolicy/frameless/forged-edge behaviour.
